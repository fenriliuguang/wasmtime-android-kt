;; L-FS-APPEND: wasi:filesystem/types@0.3.0 [method]descriptor.append-via-stream
;; Official: func(data: stream<u8>) -> future<result<_, error-code>>.
;; Guest drops futures. Two appends "P3" then "FS" so the host must not truncate.
;; Guest: get-directories → open-at("p3fs.txt") → append P3 → append FS → read P3FS.
(component
  (import "wasi:filesystem/types@0.3.0" (instance $types
    ...
    (export "[method]descriptor.append-via-stream"
  ...
  (alias export $types "[method]descriptor.append-via-stream" (func $append-via-stream))
  ...
  (core module $m
    ...
    (import "" "append-via-stream" (func $append-via-stream (param i32 i32) (result i32)))
    ...
    (func (export "run") (result i32)
      ...
      (local.set $fut (call $append-via-stream (local.get $desc) (local.get $r)))

      (local.set $status (call $stream.write (local.get $w) (i32.const 16) (i32.const 2)))
      ...
      (local.set $status (call $stream.write (local.get $w) (i32.const 18) (i32.const 2)))
      (if (i32.eq (local.get $status) (i32.const -1))
        (then unreachable))
      (call $stream.drop-writable (local.get $w))
      ;; Drop write futures without future.read. Host joins the previous
      ;; append before starting the next, then joins again in read-via-stream.
      (call $future.drop-readable (local.get $fut))
  ...
  (core func $open_at_lower
    (canon lower (func $open-at)
      (memory $libc "mem")
      (realloc (func $libc "realloc"))))
  (core func $append_lower (canon lower (func $append-via-stream) (memory $libc "mem")))
  ...
  (core instance $i (instantiate $m
    (with "" (instance
      ...
      (export "append-via-stream" (func $append_lower))
