;; WASI 0.3 package smoke: wasi:cli/stdout@0.3.0#write-via-stream
;; Transitional signature (matches root `take`): stream<u8> -> future<u32> byte count.
;; Official WIT uses future<result<_, error-code>>; enum result deferred.
;; Guest: stream.new → write-via-stream(readable) → write "OUT\n" → drop-writable → future.read.
(component
  (type $st (stream u8))
  (type $ft (future u32))
