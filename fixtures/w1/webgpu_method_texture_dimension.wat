...
;; WIT: dimension: func() -> gpu-texture-dimension. Host returns d2; harness 1.
;; L2 described texture handle → Dawn dimension (stub 1×1 when get-texture rep=0).
(component
  (import "wasi:webgpu/webgpu@0.3.0-rc.2" (instance $webgpu
    (type $texdim (enum "d1" "d2" "d3"))
    (export "gpu-texture-dimension" (type $gpu-texture-dimension (eq $texdim)))
    (export "gpu-texture" (type $gpu-texture (sub resource)))
    (type $borrow-tex (borrow $gpu-texture))
    (type $ty-fn (func (param "self" $borrow-tex) (result $gpu-texture-dimension)))
    (export "[method]gpu-texture.dimension" (func (type $ty-fn)))
    (type $own-tex (own $gpu-texture))
    (export "get-texture" (func (result $own-tex)))
  ))
  (alias export $webgpu "get-texture" (func $get-tex))
  (alias export $webgpu "[method]gpu-texture.dimension" (func $meth))
