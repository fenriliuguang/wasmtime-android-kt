...
;; Guest constructs record then values; harness 1. L2 unused (lift-only).
(component
  (import "wasi:webgpu/webgpu@0.3.0-rc.2" (instance $webgpu
    ...
    (type $list-opt-u64 (list $opt-u64))
    (type $meth-ty (func (param "self" $borrow-rec) (result $list-opt-u64)))
    (export "[method]record-option-gpu-size64.values" (func (type $meth-ty)))
  ))
  (alias export $webgpu "[constructor]record-option-gpu-size64" (func $ctor))
  (alias export $webgpu "[method]record-option-gpu-size64.values" (func $meth))
