...
;; WIT: sample-count: func() -> u32. Host returns 1; harness 1.
;; L2 described texture handle → sample-count (stub 1×1 when get-texture rep=0).
(component
  (import "wasi:webgpu/webgpu@0.3.0-rc.2" (instance $webgpu
    ...
    (export "[method]gpu-texture.sample-count" (func (type $ty)))
    (type $own-tex (own $gpu-texture))
    (export "get-texture" (func (result $own-tex)))
  ))
  (alias export $webgpu "get-texture" (func $get-tex))
  (alias export $webgpu "[method]gpu-texture.sample-count" (func $meth))
