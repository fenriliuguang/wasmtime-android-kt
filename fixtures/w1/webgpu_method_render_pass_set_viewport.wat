;; S6+: get-pass + [method]gpu-render-pass-encoder.set-viewport
;; WIT: set-viewport: func(x, y, width, height, min-depth, max-depth: f32)
;; Guest passes 0,0,1,1,0,1; run returns harness 1. L2 unused (no new JNI).
;; get-pass is a test constructor only (not product WIT).
(component
  (import "wasi:webgpu/webgpu@0.3.0-rc.2" (instance $webgpu
    (export "gpu-render-pass-encoder" (type $gpu-render-pass-encoder (sub resource)))
    (type $borrow-pass (borrow $gpu-render-pass-encoder))
    (type $set-ty (func
      (param "self" $borrow-pass)
      (param "x" f32)
      (param "y" f32)
      (param "width" f32)
      (param "height" f32)
      (param "min-depth" f32)
      (param "max-depth" f32)))
    (export "[method]gpu-render-pass-encoder.set-viewport" (func (type $set-ty)))
    (type $own-pass (own $gpu-render-pass-encoder))
    (export "get-pass" (func (result $own-pass)))
  ))
  (alias export $webgpu "get-pass" (func $get-pass))
  (alias export $webgpu "[method]gpu-render-pass-encoder.set-viewport" (func $set-vp))

  (core func $gp_lower (canon lower (func $get-pass)))
  (core func $sv_lower (canon lower (func $set-vp)))

  (core module $m
    (import "" "get-pass" (func $get-pass (result i32)))
    (import "" "set-vp" (func $set-vp (param i32 f32 f32 f32 f32 f32 f32)))
    (func (export "run") (result i32)
      (call $set-vp
        (call $get-pass)
        (f32.const 0) (f32.const 0)
        (f32.const 1) (f32.const 1)
        (f32.const 0) (f32.const 1))
      (i32.const 1)
    )
  )
  (core instance $i (instantiate $m
    (with "" (instance
      (export "get-pass" (func $gp_lower))
      (export "set-vp" (func $sv_lower))
    ))
  ))
  (func (export "run") async (result u32)
    (canon lift (core func $i "run"))
  )
)
