;; S6+: get-render-pipeline + [method]gpu-render-pipeline.set-label
;; Guest passes empty label; run returns harness 1.
(component
  (import "wasi:webgpu/webgpu@0.3.0-rc.2" (instance $webgpu
    (export "gpu-render-pipeline" (type $gpu-render-pipeline (sub resource)))
    (type $borrow-res (borrow $gpu-render-pipeline))
    (type $set-ty (func
      (param "self" $borrow-res)
      (param "label" string)))
    (export "[method]gpu-render-pipeline.set-label" (func (type $set-ty)))
    (type $own-res (own $gpu-render-pipeline))
    (export "get-render-pipeline" (func (result $own-res)))
  ))
  (alias export $webgpu "get-render-pipeline" (func $get-res))
  (alias export $webgpu "[method]gpu-render-pipeline.set-label" (func $set-label))
