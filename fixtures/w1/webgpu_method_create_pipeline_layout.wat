;; W3+: get-device + [method]gpu-device.create-pipeline-layout (sync; host-fixed empty bind-group-layouts).
(component
  (import "wasi:webgpu/webgpu@0.3.0-rc.2" (instance $webgpu
    (export "gpu-device" (type $gpu-device (sub resource)))
    (export "get-device" (func (result (own $gpu-device))))
    (export "[method]gpu-device.create-pipeline-layout"
  ...
  (core module $m
    ...
    (import "" "create-pll" (func $create-pll (param i32) (result i32)))
    (func (export "run") (result i32)
      (local $device i32)
      (local.set $device (call $get-device))
      (call $create-pll (local.get $device))
    )
  )
  (core func $gd_lower (canon lower (func $get-device)))
  (core func $cp_lower (canon lower (func $create-pll)))
  (core instance $i (instantiate $m
    (with "" (instance
      (export "get-device" (func $gd_lower))
      (export "create-pll" (func $cp_lower))
    ))
  ))
