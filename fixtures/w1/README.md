# W1/W2 wasi:webgpu request-adapter fixture
...
**Transitional:** host registers flat `request-adapter` (not `[method]gpu.request-adapter`). **W2:** true CM async (`func_wrap_concurrent` + oneshot yield); pump via `run_concurrent` / `callRunConcurrent`. Experimental flat sync path unchanged. Not full option/resource compliance.
