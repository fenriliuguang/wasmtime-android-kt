...
| `webgpu_method_create_bind_group_layout.wasm` | `get-device` + **`[method]gpu-device.create-bind-group-layout` sync** `own<gpu-bind-group-layout>` | `run: async func() -> u32` | construct device → create-bind-group-layout (one uniform buffer entry, binding=0) → drop own; harness returns 1 |
...
| `webgpu_method_create_bind_group.wasm` | `get-device` + `get-bind-group-layout` + **`[method]gpu-device.create-bind-group` sync** `own<gpu-bind-group>` | `run: async func() -> u32` | construct device + layout → create-bind-group (empty entries, label="l2") → drop own; harness returns 1 |
