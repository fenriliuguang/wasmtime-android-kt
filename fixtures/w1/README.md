...
| `webgpu_method_request_adapter.wasm` | `get-gpu` + **`[method]gpu.request-adapter` async** `option<own<gpu-adapter>>` | `run: async func() -> u32` | construct gpu → method request-adapter (L2 described; options some xr-compatible=true; power-preference + force-fallback + feature-level forwarded when some) → drop own; harness returns 1 |
...
| `webgpu_method_request_device.wasm` | `get-adapter` + **`[method]gpu-adapter.request-device` async** `result<own<gpu-device>, request-device-error>` | `run: async func() -> u32` | construct adapter → method request-device (L2 described; descriptor some required-features=`core-features-and-limits`+`depth-clip-control` + default-queue.label=`l2`; required-limits + device label forwarded when some) → drop own on ok; harness returns 1 |
...
| `webgpu_method_request_device_stage_storage_limits.wasm` | `get-adapter` + `record-option-gpu-size64` + **`[method]gpu-adapter.request-device`** | `run: async func() -> u32` | add stage-only `max-storage-*-in-vertex-stage` / `fragment-stage`=4 → request-device some(required-limits + label=`l2`) → drop own on ok; harness returns 1 |
...
| `webgpu_method_begin_render_pass.wasm` | `get-encoder` + `get-texture-view` + **`[method]gpu-command-encoder.begin-render-pass` sync** `own<gpu-render-pass-encoder>` | `run: async func() -> u32` | construct encoder+views → begin (two color clears 0,0,0,1 and 1,0,0,1 + depth-stencil) → drop owns; harness returns 1 |
...
| `webgpu_method_dawn_guest_compute.wasm` | `get-device` → **`[method]gpu-device.create-shader-module`** + **`[method]gpu-device.create-buffer`** + **`[method]gpu-device.create-bind-group-layout`** + **`[method]gpu-device.create-pipeline-layout`** + **`[method]gpu-device.create-compute-pipeline`** + **`[method]gpu-device.create-bind-group`** + encoder + queue + begin-compute-pass + **`[method]gpu-compute-pass-encoder.set-pipeline`** + **`[method]gpu-compute-pass-encoder.set-bind-group`** + **`[method]gpu-compute-pass-encoder.dispatch-workgroups`** + end + finish + submit | `run: async func() -> u32` | WG-6: BGL storage + bind-group + compute pipeline + set-bind-group + dispatch 1×1×1 + submit (not empty pass); harness returns 1 |
| `webgpu_method_dawn_render_slice.wasm` | `get-device` → **`[method]gpu-device.create-buffer`** + **`[method]gpu-device.create-texture`** + **`[method]gpu-texture.create-view`** + **`[method]gpu-device.create-command-encoder`** + **`[method]gpu-device.queue`** + **`[method]gpu-command-encoder.begin-render-pass`** + **`[method]gpu-render-pass-encoder.end`** + **`[method]gpu-command-encoder.finish`** + **`[method]gpu-queue.submit`** | `run: async func() -> u32` | D1 cite: buffer + 1×1 RGBA8 render texture + view none + encoder + one color clear pass (no depth) + submit; harness returns 1 |
| `webgpu_method_dawn_guest_render.wasm` | `get-device` → **`[method]gpu-device.create-shader-module`** + **`[method]gpu-device.create-buffer`** + **`[method]gpu-device.create-render-pipeline`** + **`[method]gpu-device.create-texture`** + **`[method]gpu-texture.create-view`** + encoder + queue + begin-render-pass + **`[method]gpu-render-pass-encoder.set-pipeline`** + **`[method]gpu-render-pass-encoder.set-vertex-buffer`** + **`[method]gpu-render-pass-encoder.draw`** + end + finish + submit | `run: async func() -> u32` | WG-6: VERTEX float32x3 + draw(3) + submit (not 1×1 color-clear cite, not vertex_index); harness returns 1 |
| `webgpu_method_dawn_guest_canvas_present.wasm` | `get-gpu` + **`[method]gpu.get-preferred-canvas-format`** + `get-canvas-context` + `get-device` + **`[method]gpu-canvas-context.configure`** + shader + VERTEX buffer + pipeline + **`[method]gpu-canvas-context.get-current-texture`** + create-view + draw(3) + submit | `run: async func() -> u32` | WG-6: guest queries preferred format then draws a swapchain frame (not host clear, not create-texture 1×1); harness returns 1 |
| `webgpu_method_create_buffer.wasm` | `get-device` + **`[method]gpu-device.create-buffer` sync** `own<gpu-buffer>` | `run: async func() -> u32` | construct device → create-buffer (`gpu-buffer-descriptor` size=4 COPY_DST\|VERTEX, mapped-at-creation=true, label=l2) → drop own; harness returns 1 |
| `webgpu_method_create_texture.wasm` | `get-device` + **`[method]gpu-device.create-texture` sync** `own<gpu-texture>` | `run: async func() -> u32` | construct device → create-texture (`gpu-texture-descriptor` 1×1×1 mip=2 sample=1 d2 rgba8unorm RENDER_ATTACHMENT, view-formats=[rgba8unorm], label=l2) → drop own; harness returns 1 |
| `webgpu_method_create_sampler.wasm` | `get-device` + **`[method]gpu-device.create-sampler` sync** `own<gpu-sampler>` | `run: async func() -> u32` | construct device → create-sampler (`option<gpu-sampler-descriptor>` some: address-mode-u/v=repeat, w=clamp-to-edge, mag/min/mipmap-filter=linear, lod 0..32, compare=never) → drop own; harness returns 1 |
| `webgpu_method_canvas_context_configure.wasm` | `get-canvas-context` + `get-device` + **`[method]gpu-canvas-context.configure` sync** void | `run: async func() -> u32` | construct context+device → configure (format=rgba8unorm, view-formats=[rgba8unorm], color-space=srgb, tone-mapping=standard, alpha-mode=premultiplied, L2 described device/format/usage + leftovers); harness returns 1 |
...
| `webgpu_method_create_shader_module.wasm` | `get-device` + **`[method]gpu-device.create-shader-module` sync** `own<gpu-shader-module>` | `run: async func() -> u32` | construct device → create-shader-module (WGSL `fn l2`, compilation-hints entry-point=l2, label=l2) → drop own; harness returns 1 |
...
| `webgpu_method_texture_create_view.wasm` | `get-texture` + **`[method]gpu-texture.create-view` sync** `own<gpu-texture-view>` | `run: async func() -> u32` | construct texture → create-view (`option<gpu-texture-view-descriptor>` some: format=rgba8unorm, dimension=d2, aspect=all, mip 0/1, array layers 0/1) → drop own; harness returns 1 |
...
| `webgpu_method_create_render_pipeline.wasm` | `get-device` + `get-shader-module` + **`[method]gpu-device.create-render-pipeline` sync** `own<gpu-render-pipeline>` | `run: async func() -> u32` | construct device + shader → create-render-pipeline (vs_main, one float32x3 buffer stride=12, fragment rgba8unorm write-mask=all, depth-stencil stencil-front/back + masks + bias, layout auto, label="l2", primitive cull-mode=back) → drop own; harness returns 1 |
...
| `webgpu_method_create_compute_pipeline_constants.wasm` | `get-device` + `get-shader-module` + `[constructor]record-gpu-pipeline-constant-value` + `add` + **`[method]gpu-device.create-compute-pipeline`** | `run: async func() -> u32` | construct record add key=`c` value=`1.0` → create-compute-pipeline (`compute.constants` some); harness returns 1 |
...
wasm-tools parse fixtures/w1/webgpu_method_request_device_stage_storage_limits.wat -o fixtures/w1/webgpu_method_request_device_stage_storage_limits.wasm
wasm-tools validate --features=cm-async,component-model fixtures/w1/webgpu_method_request_device_stage_storage_limits.wasm
...
wasm-tools parse fixtures/w1/webgpu_method_dawn_guest_compute.wat -o fixtures/w1/webgpu_method_dawn_guest_compute.wasm
wasm-tools validate --features=cm-async,component-model fixtures/w1/webgpu_method_dawn_guest_compute.wasm
wasm-tools parse fixtures/w1/webgpu_method_dawn_render_slice.wat -o fixtures/w1/webgpu_method_dawn_render_slice.wasm
wasm-tools validate --features=cm-async,component-model fixtures/w1/webgpu_method_dawn_render_slice.wasm
wasm-tools parse fixtures/w1/webgpu_method_dawn_guest_render.wat -o fixtures/w1/webgpu_method_dawn_guest_render.wasm
wasm-tools validate --features=cm-async,component-model fixtures/w1/webgpu_method_dawn_guest_render.wasm
wasm-tools parse fixtures/w1/webgpu_method_dawn_guest_canvas_present.wat -o fixtures/w1/webgpu_method_dawn_guest_canvas_present.wasm
wasm-tools validate --features=cm-async,component-model fixtures/w1/webgpu_method_dawn_guest_canvas_present.wasm
...
wasm-tools parse fixtures/w1/webgpu_method_create_compute_pipeline_constants.wat -o fixtures/w1/webgpu_method_create_compute_pipeline_constants.wasm
wasm-tools validate --features=cm-async,component-model fixtures/w1/webgpu_method_create_compute_pipeline_constants.wasm
