...
| `webgpu_method_dawn_guest_canvas_present.wasm` | `get-canvas-context` + `get-device` + **`[method]gpu-canvas-context.configure`** + shader + VERTEX buffer + pipeline + **`[method]gpu-canvas-context.get-current-texture`** + create-view + draw(3) + submit | `run: async func() -> u32` | WG-6: guest-drawn swapchain frame (not host clear, not create-texture 1×1); harness returns 1 |
