...
(component
  (import "wasi:webgpu/webgpu@0.3.0-rc.2" (instance $webgpu
    ...
    (export "[method]gpu-adapter-info.subgroup-min-size" (func (type $ty)))
  ...
  (alias export $webgpu "[method]gpu-adapter-info.subgroup-min-size" (func $min))

  (core func $gi_lower (canon lower (func $get-info)))
  (core func $m_lower (canon lower (func $min)))

  (core module $m
    (import "" "get-info" (func $get-info (result i32)))
    (import "" "min" (func $min (param i32) (result i32)))
    (func (export "run") (result i32)
      (drop (call $min (call $get-info)))
      (i32.const 1)
    )
  )
