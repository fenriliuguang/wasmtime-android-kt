...
(component
  (import "wasi:webgpu/webgpu@0.3.0-rc.2" (instance $webgpu
    ...
    (type $set-ty (func
      (param "self" $borrow-qs)
      (param "label" string)))
    (export "[method]gpu-query-set.set-label" (func (type $set-ty)))
    (type $own-qs (own $gpu-query-set))
    (export "get-query-set" (func (result $own-qs)))
  ))
  (alias export $webgpu "get-query-set" (func $get-qs))
  (alias export $webgpu "[method]gpu-query-set.set-label" (func $set-label))
  ...
  (core module $m
    ...
    (func (export "run") (result i32)
      (call $set-label (call $get-qs) (i32.const 0) (i32.const 0))
      (i32.const 1)
    )
  )
