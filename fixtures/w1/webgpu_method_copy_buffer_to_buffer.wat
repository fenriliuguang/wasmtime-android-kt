;; W3+: get-encoder + [method]gpu-command-encoder.copy-buffer-to-buffer
;; (self, source u32, destination u32; void). Returns stub 31.
(component
  (import "wasi:webgpu/webgpu@0.3.0-rc.2" (instance $webgpu
    (export "gpu-command-encoder" (type $gpu-command-encoder (sub resource)))
    (export "get-encoder" (func (result (own $gpu-command-encoder))))
    (export "[method]gpu-command-encoder.copy-buffer-to-buffer"
      (func (param "self" (borrow $gpu-command-encoder))
            (param "source" u32) (param "destination" u32)))
  ))
  (alias export $webgpu "get-encoder" (func $get-encoder))
  (alias export $webgpu "[method]gpu-command-encoder.copy-buffer-to-buffer" (func $copy))

  (core module $m
    (import "" "get-encoder" (func $get-encoder (result i32)))
    (import "" "copy" (func $copy (param i32 i32 i32)))
    (func (export "run") (result i32)
      (local $encoder i32)
      (local.set $encoder (call $get-encoder))
      (call $copy (local.get $encoder) (i32.const 31) (i32.const 31))
      (i32.const 31)
    )
  )
  (core func $ge_lower (canon lower (func $get-encoder)))
  (core func $c_lower (canon lower (func $copy)))
  (core instance $i (instantiate $m
    (with "" (instance
      (export "get-encoder" (func $ge_lower))
      (export "copy" (func $c_lower))
    ))
  ))
  (func (export "run") async (result u32)
    (canon lift (core func $i "run"))
  )
)
