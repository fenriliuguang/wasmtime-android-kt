...
| Dawn C API (ND-DEFAULT product `.so`) | Google Android prebuilt SHA `bddf1a04f7c262107a9aae301c45fc49e15c7fef` (press `--prebuilt`). Recipe [`scripts/build-dawn-c-android.py`](scripts/build-dawn-c-android.py) links `libwebgpu_dawn.so` (`webgpu.h` exports). BSD-3-Clause: [`native/third_party/dawn-c/LICENSE`](native/third_party/dawn-c/LICENSE). **Do not git-add the `.so`.** From **0.1.2-SNAPSHOT** the press job packs that binary into the `host-dawn` AAR (`0.1.1` packed a different `--build`). Default APK excludes androidx `libwebgpu_c_bundled.so`. |
