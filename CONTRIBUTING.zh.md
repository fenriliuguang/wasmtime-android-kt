...
短命分支 + PR；不要直推 `main`。英文为正文。`release/0.1.0` 是按下 `0.1.0` 前的唯一长寿例外，且从不在该分支发包。
...
CI（`main` / `release/0.1.0`）：host `cargo test`、JVM compile、NDK `.so`、AAR assemble。真机 instruments 与仓外 example 在本机跑，作为 GitHub Environment `release` 的 press 门禁。发包仅 `main` 上的 `v*` 标签或从 `main` 手动触发。
