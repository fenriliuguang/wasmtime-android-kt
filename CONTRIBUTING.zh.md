...
CI（`main` / `release/0.1.0`）：host `cargo test`、JVM compile、NDK `.so`、AAR assemble。仅改 `*.md` / `changelog/` 时这四项 skipped，聚合检查 `CI` 仍绿。真机 instruments 与仓外 example 在本机跑，作为 GitHub Environment `release` 的发包门禁。当前 GAV **`0.1.1`**：发包编 wasmtime `.so` 与 `libwebgpu_dawn.so`，缺 arm64 则失败。后续发包仅 `main` 上的 `v*` 标签或从 `main` 手动触发。
