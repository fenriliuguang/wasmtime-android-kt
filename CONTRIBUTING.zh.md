...
CI（`main` / `release/0.1.0`）：host `cargo test`、JVM compile、NDK `.so`、AAR assemble。真机 instruments 与仓外 example 在本机跑，作为 GitHub Environment `release` 的发包门禁。`0.1.0` 已发布。后续发包仅 `main` 上的 `v*` 标签或从 `main` 手动触发。
