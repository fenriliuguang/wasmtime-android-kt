...
混淆必须吃到 AAR 的 `consumer-rules.pro`。sockets / 出站 HTTP 需要 Android **INTERNET**。
...
无 GPU / 自带后端：`…:runtime:0.1.3-SNAPSHOT`。只要 Dawn host：`…:host-dawn:0.1.3-SNAPSHOT`。不要直接依赖 `runtime-api` / `runtime-jni`（`runtime` 的传递依赖）。不要依赖 `:smoke-app`。

`host-dawn` / `android-webgpu` **0.1.3-SNAPSHOT** 打进 Press 钉的 `libwebgpu_dawn.so`（NativeGpu：Google Android `--prebuilt` SHA `bddf1a04…`，即真机绿的那份）。**不要用 `0.1.1` 跑 GPU**（那份是另一套 `--build` Dawn）。应用 **不必** 自己编 Dawn，也 **不必** vendor `androidx.webgpu`。`runtime` 不含 Dawn `.so`。没有这份 `.so` 时 NativeGpu 走 **table-backed**；**发包** 缺 arm64 文件则失败。
...
端到端示例（打包 guest、加载、上屏）：[wasmtime-android-kt-examples](https://github.com/fenriliuguang/wasmtime-android-kt-examples)。本仓不内嵌该 app。仓外 cube：`.\scripts\verify-examples-gate.ps1`（includeBuild，本地开发）。发包 AAR 门禁：`python3 .\scripts\verify-press-aar.py --assemble`（本仓，不依赖 examples）。
