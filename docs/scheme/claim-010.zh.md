...
第三方可以依赖：钉住的 **`wasi:webgpu@0.3.0-rc.2` 绝大多数 `[method]` 能 instantiate**、webgpu 应用需要的 **WASI 0.3 IO/网络子集**、以及 **完整 `wasi-gfx` 上屏循环**（产品 adapter/device + vsync）。在 **P010-GFXB** / **P010-GFXV** 前，循环只是两帧预缓冲 + fixture `get-device`。androidx 空洞与 named-only WASI 剩项公开列出，不当静默丢字段。
...
| gfx | `surface@0.2.0` + 两帧预缓冲 present；**完整循环待 P010-GFXB / P010-GFXV** | 完整桌面 gfx |
