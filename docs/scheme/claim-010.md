...
**This is not a compliance claim.** No WebGPU CTS. No “full WASI 0.3”. No this-repo 1.0. See [`non-goals.md`](non-goals.md). Product subset for **0.1.x**. Current GAV **`0.1.1`** packs `libwebgpu_dawn.so` into published `host-dawn` / `android-webgpu`. Publishing CI: [`.github/workflows/publish.yml`](../../.github/workflows/publish.yml).
...
Default consume is Dawn C / NativeGpu. Maven **0.1.1** packs `libwebgpu_dawn.so` in `host-dawn`. Remaining pin `[method]`s call `webgpu.h` when that `.so` is loaded; Cloud CI assemble without the recipe stays table-backed ([`../mapping/gap-webgpu-native-dawn.md`](../mapping/gap-webgpu-native-dawn.md)). Cube is demo evidence only.
