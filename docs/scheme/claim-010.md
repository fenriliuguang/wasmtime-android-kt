...
**This is not a compliance claim.** No WebGPU CTS. No “full WASI 0.3”. No this-repo 1.0. See [`non-goals.md`](non-goals.md). Product subset for **0.1.x**. Current GAV **`0.1.3-SNAPSHOT`** packs device-green `--prebuilt` `libwebgpu_dawn.so` into published `host-dawn` / `android-webgpu`. Publishing CI: [`.github/workflows/publish.yml`](../../.github/workflows/publish.yml).
...
| Remaining pin methods | **Dawn** when `.so` loads | texture / sampler / compute / copies (origin / mip / aspect / layout) / **map + mapped-range** / query / **bundle recording** / indexed-indirect / viewport / `write-texture` / work-done / destroy / adapter `GetInfo` / **GetLimits** / buffer·texture getters / `set-bind-group` dynamic offsets. Cloud / missing `.so` stays **Table** (`gpu-supported-limits.*` return `1`). Leftovers listed in the gap **Remaining Table** auto queue ([`nativegpu-remaining.md`](nativegpu-remaining.md), [#317](https://github.com/fenriliuguang/wasmtime-android-kt/issues/317)): compilation-info, pop-error-scope payload, uncaptured/lost, most labels. |
