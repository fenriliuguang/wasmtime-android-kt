...
3. Current coordinate: **`0.1.0`** (`gradle.properties` → `wasmtime.android.version`). **Pressed.** Subsequent bumps follow the `0.x` rules below. Publishing CI: [`.github/workflows/publish.yml`](../../.github/workflows/publish.yml). Do not press when secrets or arm64 jniLibs are missing.
