...
终态 resource 方法名属 **W3**。扁平 `adapter-request-device` 真 async **已交付**（同提案 instance；L2 `exp_adapter_request_device`）。扁平 `device-get-queue` sync **已交付**（同提案 instance；L2 `exp_device_get_queue`；非 `[method]gpu-device.queue`）。
...
| Guest | `fixtures/w1/webgpu_request_adapter.{wat,wasm}` · `webgpu_request_device.{wat,wasm}` · `webgpu_device_get_queue.{wat,wasm}` |
| Native smoke | `native/tests/wasi_webgpu_request_adapter.rs`（stub u32=7）· `wasi_webgpu_request_device.rs`（stub device=11）· `wasi_webgpu_device_get_queue.rs`（stub queue=13） |
| 仪器孪生 | `WasiWebGpuRequestAdapterInstrumentedTest` · `WasiWebGpuRequestDeviceInstrumentedTest` · `WasiWebGpuDeviceGetQueueInstrumentedTest` |
...
- W1 已交付；W2：`request-adapter` + 扁平 `adapter-request-device` 真 async 已交付。W3 首片：扁平 `device-get-queue` sync 已交付（非 `[method]`）。  
