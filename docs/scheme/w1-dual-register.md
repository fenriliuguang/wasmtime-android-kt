...
> **W3+ 本片：** 已注册 **`[method]gpu-compute-pass-encoder.dispatch-workgroups`**（**sync** void；JNI 忽略 Guest counts；host 先 set-pipeline + 空 bind-group 0 再 dispatch 1×1×1）。
