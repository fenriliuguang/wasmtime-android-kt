...
> **W3+ 本片：** 已注册 **`[method]gpu-render-pass-encoder.set-vertex-buffer`**（**sync** void；JNI 忽略 Guest stub buffer；host 固定 VERTEX buffer slot 0 + Cpu 离屏 view）。
