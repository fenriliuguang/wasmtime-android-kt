...
> **W3+ 本片：** 已注册 **`[method]gpu-render-pass-encoder.set-pipeline`**（**sync** void，复用 `gpu-render-pass-encoder` / `get-pass`；host 固定 triangle pipeline + Cpu 离屏 view；Guest stub pipeline u32）。
