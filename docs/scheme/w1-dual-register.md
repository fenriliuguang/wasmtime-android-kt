...
> **W3+ 本片：** 已注册 **`[method]gpu-device.create-render-pipeline`**（**sync**，host 固定 stub WGSL + triangle RGBA8；返回仍为过渡 u32，非 `gpu-render-pipeline` resource）。
