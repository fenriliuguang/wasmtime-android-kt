...
> **W3+ 本片：** 已注册 **`[method]gpu-device.create-buffer`**（**sync**，复用 `gpu-device` / `get-device`；host 固定 descriptor；返回仍为过渡 u32，非 `gpu-buffer` resource）。
