...
> **W3+ 本片：** 已注册 **`[method]gpu-compute-pass-encoder.set-pipeline`**（**sync** void，复用 `gpu-compute-pass-encoder` / `get-compute-pass`；host 固定 stub shader + 空 layout compute pipeline；Guest stub pipeline u32）。
