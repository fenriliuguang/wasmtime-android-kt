...
> **W3+ 本片：** 已注册 WIT `gpu-texture` + `get-texture` + **`[method]gpu-texture.create-view`**（**sync**，host 固定 1×1；返回仍为过渡 u32，非 `gpu-texture-view` resource）。
