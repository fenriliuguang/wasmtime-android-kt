...
> **W3+ 本片：** 已注册 **`[method]gpu-command-encoder.begin-compute-pass`**（**sync**，复用 `gpu-command-encoder` / `get-encoder`；无 Guest descriptor；返回仍为过渡 u32，非 `gpu-compute-pass-encoder` resource）。
