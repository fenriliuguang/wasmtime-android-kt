...
> **W3 本片：** 已注册 WIT `gpu-adapter` + `get-adapter` + **`[method]gpu-adapter.request-device`**（真 async，resource self；返回仍为过渡 u32，非 `result<gpu-device, …>`）。扁平 `adapter-request-device` 仍保留。
