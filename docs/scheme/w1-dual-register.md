...
> **W3 首片：** 已双注册过渡扁平 `device-get-queue`（**sync** getter，同 L2 u32）。**S1：** `[method]gpu-device.queue` 返回 **`own<gpu-queue>`**。**S2：** `[method]gpu.request-adapter` 返回 **`option<own<gpu-adapter>>`**（真 async；options=none）。**S3：** `[method]gpu-adapter.request-device` 返回 **`result<own<gpu-device>, request-device-error>`**（真 async；descriptor=none）。  
