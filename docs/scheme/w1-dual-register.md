...
> **W3+ 本片：** 已注册 **`[method]gpu-device.create-bind-group-layout`**（**sync**，复用 `gpu-device` / `get-device`；host 固定空 entries；返回仍为过渡 u32，非 `gpu-bind-group-layout` resource）。
