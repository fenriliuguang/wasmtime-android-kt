...
> **W3+ 本片：** 已注册 **`[method]gpu-device.create-bind-group`**（**sync**，host 先建空 BGL 再空 entries；返回仍为过渡 u32，非 `gpu-bind-group` resource）。
