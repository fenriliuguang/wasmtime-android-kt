...
> **W3+ 本片：** 已注册 **`[method]gpu-queue.write-buffer`**（**sync** void，复用 `gpu-queue` / `get-queue`；host 固定 4 字节；Guest stub buffer u32）。
