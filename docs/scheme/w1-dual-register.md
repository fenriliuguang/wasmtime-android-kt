...
> **W2 进度：** `request-adapter` 与扁平 `adapter-request-device` 真 async **均已交付**（`feat/webgpu-w2-async-request-adapter` · `feat/webgpu-w2-async-request-device`）。终态 `[method]` / resource 表属 W3。
...
终态 resource 方法名属 **W3**。扁平 `adapter-request-device` 真 async **已交付**（同提案 instance；L2 `exp_adapter_request_device`）。
...
| Guest | `fixtures/w1/webgpu_request_adapter.{wat,wasm}` · `webgpu_request_device.{wat,wasm}` |
| Native smoke | `native/tests/wasi_webgpu_request_adapter.rs`（stub u32=7）· `wasi_webgpu_request_device.rs`（stub device=11） |
| 仪器孪生 | `WasiWebGpuRequestAdapterInstrumentedTest` · `WasiWebGpuRequestDeviceInstrumentedTest` |
