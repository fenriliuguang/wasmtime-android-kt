...
> **W3+ 本片：** 已注册 **`[method]gpu-queue.write-texture`**（**sync** void，复用 `gpu-queue` / `get-queue`；host 固定 1×1 COPY_DST；Guest stub texture u32）。
