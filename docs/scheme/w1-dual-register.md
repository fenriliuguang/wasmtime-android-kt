...
> **W3 首片：** 已双注册过渡扁平 `device-get-queue`（**sync** getter，同 L2 u32）。**S1：** `[method]gpu-device.queue` 返回 **`own<gpu-queue>`**。**S2：** `[method]gpu.request-adapter` 返回 **`option<own<gpu-adapter>>`**（真 async；options=none）。**S3：** `[method]gpu-adapter.request-device` 返回 **`result<own<gpu-device>, request-device-error>`**（真 async；descriptor=none）。**S4：** `[method]gpu-device.create-buffer` 接受 **`gpu-buffer-descriptor`**，返回 **`own<gpu-buffer>`**。**S5：** `[method]gpu-queue.submit` 接受 **`list<borrow<gpu-command-buffer>>`**。  
> **W3：** 已双注册过渡扁平 `device-create-command-encoder`（**sync**，同 L2 u32）。**S6：** 终态 `[method]gpu-device.create-command-encoder` **已交付**（`option<gpu-command-encoder-descriptor>` = none → **`own<gpu-command-encoder>`**）。  
> **W3：** 已双注册过渡扁平 `command-encoder-finish`（**sync**，同 L2 u32）。**S7：** 终态 `[method]gpu-command-encoder.finish` **已交付**（`option<gpu-command-buffer-descriptor>` = none → **`own<gpu-command-buffer>`**）。  
> **W3：** 已双注册过渡扁平 `queue-submit1`（**sync** void，同 L2 单 buffer u32）。**S5：** 终态 `[method]gpu-queue.submit` **已交付**（`list<borrow<gpu-command-buffer>>`；Guest 单元素 list；L2 仍 host-fixed `submit1`）。  
...
> **W3+：** 已注册 **`[method]gpu-device.create-buffer`**（**S4：** Guest 传入 `gpu-buffer-descriptor`，返回 **`own<gpu-buffer>`**）。  
> **W3+：** 已注册 **`[method]gpu-device.create-texture`**（**sync**，host 固定 1×1 RGBA8；返回仍为过渡 u32，非 `gpu-texture` resource）。  
> **W3+：** 已注册 **`[method]gpu-device.create-sampler`**（**S8：** Guest descriptor=none → **`own<gpu-sampler>`**；L2 仍 host-fixed default sampler）。  
...
> **W3+：** 已注册 WIT `gpu-texture` + `get-texture` + **`[method]gpu-texture.create-view`**（**S8：** Guest descriptor=none → **`own<gpu-texture-view>`**；L2 仍 host-fixed 1×1）。  
...
> **W3+：** 已注册 **`[method]gpu-command-encoder.begin-compute-pass`**（**S8：** Guest descriptor=none → **`own<gpu-compute-pass-encoder>`**；L2 仍 host-default）。  
...
> **W3+：** 已注册 **`[method]gpu-render-pass-encoder.draw`**（**sync** void；JNI 忽略 Guest vertexCount；host 先 set-pipeline 再 draw(3)）。  
> **W3+：** 已注册 **`[method]gpu-compute-pass-encoder.set-bind-group`**（**sync** void；JNI 忽略 Guest stub bind-group；host 固定空 bind-group index 0）。  
> **W3+：** 已注册 **`[method]gpu-render-pass-encoder.set-bind-group`**（**sync** void；JNI 忽略 Guest stub bind-group；host 固定空 bind-group index 0 + Cpu 离屏 view）。  
> **W3+：** 已注册 **`[method]gpu-render-pass-encoder.set-vertex-buffer`**（**sync** void；JNI 忽略 Guest stub buffer；host 固定 VERTEX buffer slot 0 + Cpu 离屏 view）。  
> **W3+：** 已注册 WIT `gpu-buffer` + `get-buffer` + **`[method]gpu-buffer.map-async`**（**真 async** void；host 固定 MAP_READ 再 map；Guest stub buffer u32）。  
> **W3+ 本片：** 已注册 **`[method]gpu-buffer.unmap`**（**sync** void；host 先 map 再 unmap；Guest stub buffer u32）。
...
- W1 已交付；W2：`request-adapter` + 扁平 `adapter-request-device` 真 async 已交付。W3：扁平 `device-get-queue` + `device-create-command-encoder` + `command-encoder-finish` + `queue-submit1` + `command-encoder-begin-render-pass-clear` + `render-pass-end` sync 已交付；**`get-gpu` + `[method]gpu.request-adapter`** 真 async 已交付（非 `option<gpu-adapter>`）；**`get-adapter` + `[method]gpu-adapter.request-device`** 真 async 已交付（非 `result<gpu-device, …>`）；**`get-device` + `[method]gpu-device.queue`** sync 已交付（非 `gpu-queue` resource）；**`[method]gpu-device.create-command-encoder`** **S6 已交付**（Guest descriptor=none → `own<gpu-command-encoder>`）；**`[method]gpu-command-encoder.finish`** **S7 已交付**（Guest descriptor=none → `own<gpu-command-buffer>`）；**`[method]gpu-device.create-buffer`** **S4 已交付**（Guest `gpu-buffer-descriptor` → `own<gpu-buffer>`）；**`[method]gpu-queue.submit`** **S5 已交付**（Guest `list<borrow<gpu-command-buffer>>`）；**`[method]gpu-device.create-texture`** sync 已交付（host 固定 1×1）；**`[method]gpu-device.create-sampler`** **S8 已交付**（Guest descriptor=none → `own<gpu-sampler>`）；**`[method]gpu-texture.create-view`** **S8 已交付**（Guest descriptor=none → `own<gpu-texture-view>`）。**`[method]gpu-device.create-shader-module`** sync 已交付（host 固定 WGSL）；**`[method]gpu-queue.write-buffer`** sync 已交付（host 固定 4 字节）。**`[method]gpu-device.create-bind-group-layout`** sync 已交付（host 固定空 entries）。**`[method]gpu-device.create-pipeline-layout`** sync 已交付（host 固定空 bind-group-layouts）。**`[method]gpu-device.create-bind-group`** sync 已交付（host 固定空 BGL + 空 entries）。**`[method]gpu-device.create-render-pipeline`** sync 已交付（host 固定 stub shader + triangle）。**`[method]gpu-device.create-compute-pipeline`** sync 已交付（host 固定 stub shader + 空 pipeline-layout）。**`[method]gpu-queue.write-texture`** sync 已交付（host 固定 1×1 COPY_DST）。**`[method]gpu-command-encoder.begin-compute-pass`** **S8 已交付**（Guest descriptor=none → `own<gpu-compute-pass-encoder>`）。**`[method]gpu-compute-pass-encoder.end`** sync void 已交付。**`[method]gpu-command-encoder.copy-buffer-to-buffer`** sync void 已交付（host 固定 4 字节 copy）。**`[method]gpu-compute-pass-encoder.set-pipeline`** sync void 已交付（host 固定 stub compute pipeline）。**`[method]gpu-compute-pass-encoder.dispatch-workgroups`** sync void 已交付（host 固定 1×1×1）。**`[method]gpu-render-pass-encoder.set-pipeline`** sync void 已交付（host 固定 triangle pipeline）。**`[method]gpu-render-pass-encoder.draw`** sync void 已交付（host 固定 vertexCount 3）。**`[method]gpu-compute-pass-encoder.set-bind-group`** sync void 已交付（host 固定空 bind-group index 0）。**`[method]gpu-render-pass-encoder.set-bind-group`** sync void 已交付（host 固定空 bind-group index 0）。**`[method]gpu-render-pass-encoder.set-vertex-buffer`** sync void 已交付（host 固定 VERTEX buffer slot 0）。**`[method]gpu-buffer.map-async`** 真 async void 已交付（host 固定 MAP_READ）。**`[method]gpu-buffer.unmap`** sync void 已交付（host 先 map 再 unmap）。  
