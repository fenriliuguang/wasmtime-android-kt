...
> **W3 本片：** 已双注册过渡扁平 `render-pass-end`（**sync** void，同 L2 u32）。终态 `[method]gpu-render-pass-encoder.end` 仍属后续 W3。
...
| Guest | `fixtures/w1/webgpu_request_adapter.{wat,wasm}` · `webgpu_request_device.{wat,wasm}` · `webgpu_device_get_queue.{wat,wasm}` · `webgpu_create_command_encoder.{wat,wasm}` · `webgpu_command_encoder_finish.{wat,wasm}` · `webgpu_queue_submit.{wat,wasm}` · `webgpu_begin_render_pass.{wat,wasm}` · `webgpu_render_pass_end.{wat,wasm}` |
...
- W1 已交付；W2：`request-adapter` + 扁平 `adapter-request-device` 真 async 已交付。W3：扁平 `device-get-queue` + `device-create-command-encoder` + `command-encoder-finish` + `queue-submit1` + `command-encoder-begin-render-pass-clear` + `render-pass-end` sync 已交付（非 `[method]`）。  
