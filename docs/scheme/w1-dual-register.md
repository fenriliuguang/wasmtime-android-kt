...
> **W3+ 本片：** 已注册 **`[method]gpu-device.create-compute-pipeline`**（**sync**，host 固定 stub WGSL + 空 pipeline-layout；返回仍为过渡 u32，非 `gpu-compute-pipeline` resource）。
