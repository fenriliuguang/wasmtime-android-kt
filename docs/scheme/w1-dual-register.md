...
> **W3+ 本片：** 已注册 WIT `gpu-buffer` + `get-buffer` + **`[method]gpu-buffer.map-async`**（**真 async** void；host 固定 MAP_READ 再 map；Guest stub buffer u32）。
