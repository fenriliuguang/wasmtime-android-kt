...
| NG-6 | Publish **Maven Central** / GitHub Packages **outside** GitHub Environment `release`, or when secrets / arm64 wasmtime `.so` / arm64 `libwebgpu_dawn.so` are missing. `0.1.2-SNAPSHOT` packs device-green NativeGpu into `host-dawn`; SNAPSHOT and later GAVs still go through that Environment |
