...
- `0.1.0`：大部分钉版 `wasi:webgpu` WIT + 业务所需 IO/网络 + [gfx 帧循环 RFC](rfc-wasi-gfx-frame-loop.md)（产品 adapter/device **P010-GFXB** + vsync **P010-GFXV**；两帧预缓冲不够）。Dawn/androidx 无槽位列为已知限制。不宣称 CTS。
