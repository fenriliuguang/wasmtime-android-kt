...
L5 §7 行（出站 TCP、HTTP body/出站、G-http-ctor、产品路径 cli 错误）走 **`product-010-remaining.py`**，不是本表的「永不自动」。HTTP body `stream<u8>` 已 P010-HBODY；出站 `client.send` 已 P010-HOUT；产品面已拿掉 `[constructor]request`/`response`（P010-HCTOR）。
