...
真机现象（V2458A / Mali：连续 present 抽搐升高后 GpuThread SIGSEGV；开局转得过快）记在英文 §5。节拍同步：[`gfx-hitch-checklist.zh.md`](gfx-hitch-checklist.zh.md)。本分支：acquire 不再等上一帧 GPU；poller 上 retire；`writeBuffer` 加锁+复用 buffer。`on-frame` **不锁 60Hz**；pin `frame-event` 无时间戳，运动 delta 用 `monotonic-clock.now`。Guest 自有纹理不扫。
