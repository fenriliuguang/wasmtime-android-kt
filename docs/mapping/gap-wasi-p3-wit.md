...
| `wasi:http` | **Smoke** body `stream<u8>` + outbound GET; official `error-code` variant; empty authority → `HTTP-request-URI-invalid`; https without TLS crate → `TLS-protocol-error`; `fields` + `request`/`response` `get-headers` (immutable child); `consume-body` trailers `option` (this cut: `none`); product linker omits request/response constructors |
