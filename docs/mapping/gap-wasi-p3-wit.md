...
| `wasi:filesystem` | **Smoke** + official `error-code` variant; `..` → `access`; missing descriptor → `bad-descriptor`; r/w IO → `io` / `is-directory` (no `unknown`); `stat` / `stat-at` on sandbox descriptor; `read-directory` as a CM stream (omit `.` / `..`) |
