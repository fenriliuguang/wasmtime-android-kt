...
| **G-http-ctor** | Drop `[constructor]request` / `[constructor]response` from the **product** types surface (host supplies `request` when calling `handle`) | **Smoke** — product linker omits constructors; test linker keeps them (P010-HCTOR) |
...
| `wasi:http` | **Smoke** in-process 200 + `handle -> result<response, error-code>` + body `stream<u8>` (P010-HBODY) + outbound `client.send` HTTP/1.1 (P010-HOUT) + product linker omits `[constructor]request`/`response` (P010-HCTOR) |
