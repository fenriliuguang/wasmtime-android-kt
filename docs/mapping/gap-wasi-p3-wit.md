...
| **G-sock-shape** | `create-tcp-socket(ip-address-family) -> result<tcp-socket, error-code>`; `connect: async func(ip-socket-address) -> result<_, error-code>` (loopback still OK); data plane stays stream-plus-future | create takes family + result (`ipv4`); connect takes `()` and hard-codes `127.0.0.1` | **P1-SK2** |
...
| `wasi:sockets` | **Smoke** loopback echo + create family/result; **Goal** G-sock-shape connect |
