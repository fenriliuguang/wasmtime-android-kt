...
| Pipeline / Bind group | render/compute pipeline | **W3** `[method]gpu-device.create-bind-group-layout` / `[method]gpu-device.create-pipeline-layout` / `[method]gpu-device.create-bind-group` / `[method]gpu-device.create-render-pipeline` sync 已交付（host 固定空 entries / stub shader + triangle，仍 u32）；compute pipeline 仍后 |
| Shader module | WGSL 模块 | **W3** `[method]gpu-device.create-shader-module` sync 已交付（host 固定 WGSL，仍 u32） |
| Queue 写入 | `write-buffer` / `write-texture` / `on-submitted-work-done`（async） | **W3** `[method]gpu-queue.write-buffer` sync 已交付（host 固定 4 字节，单 buffer u32）；`write-texture` / async 仍后 |
...
| 横切 | 现状 | 目标（长期） |
|------|------|----------------|
| WIT resource + `[method]` 名 | 扁平函数 + u32；**W3 高频面已挂** `gpu` / `gpu-adapter` / `gpu-device` / `gpu-command-encoder` / `gpu-render-pass-encoder` / `gpu-queue` 及对应 `[method]`；**W3+** `[method]gpu-device.create-buffer` / `[method]gpu-device.create-texture` / `[method]gpu-device.create-sampler` / `[method]gpu-device.create-shader-module` / `[method]gpu-queue.write-buffer` / `[method]gpu-texture.create-view` / `[method]gpu-device.create-bind-group-layout` / `[method]gpu-device.create-pipeline-layout` / `[method]gpu-device.create-bind-group` / `[method]gpu-device.create-render-pipeline`（返回仍 u32 / void） | 提案 resource 表；rep 仍可 u32 对齐 L2；descriptor / `list` / `option` 仍后 |
