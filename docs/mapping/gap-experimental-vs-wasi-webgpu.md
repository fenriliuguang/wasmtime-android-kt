...
| 横切 | 现状 | 目标（长期） |
|------|------|----------------|
| WIT resource + `[method]` 名 | 扁平函数 + u32；**W3 高频面已挂** `gpu` / `gpu-adapter` / `gpu-device` / `gpu-command-encoder` / `gpu-render-pass-encoder` / `gpu-queue` 及对应 `[method]`；**W3+** `[method]gpu-device.create-buffer` / `[method]gpu-device.create-texture` / `[method]gpu-device.create-sampler` / `[method]gpu-device.create-shader-module` / `[method]gpu-queue.write-buffer` / `[method]gpu-texture.create-view` / `[method]gpu-device.create-bind-group-layout` / `[method]gpu-device.create-pipeline-layout` / `[method]gpu-device.create-bind-group` / `[method]gpu-device.create-render-pipeline` / `[method]gpu-device.create-compute-pipeline`（返回仍 u32 / void） | 提案 resource 表；rep 仍可 u32 对齐 L2；descriptor / `list` / `option` 仍后 |
