...
| Command encoding | begin-compute-pass / copy-buffer / pass set-pipeline / draw | **W3** `[method]gpu-command-encoder.begin-compute-pass` / `[method]gpu-compute-pass-encoder.end` / `[method]gpu-command-encoder.copy-buffer-to-buffer` / `[method]gpu-compute-pass-encoder.set-pipeline` / `[method]gpu-compute-pass-encoder.dispatch-workgroups` sync 已交付（无 descriptor / host 固定 4 字节 copy / stub compute pipeline / 1×1×1 dispatch，仍 u32 / void）；render set-pipeline / draw 仍后 |
