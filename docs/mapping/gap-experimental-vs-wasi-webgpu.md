...
| Buffer / Texture / Sampler | 创建、map（**async** map 相关） | **W3** `[method]gpu-device.create-buffer` / `[method]gpu-device.create-texture` / `[method]gpu-device.create-sampler` sync 已交付（host 固定 descriptor，仍 u32）；map 仍后 |
