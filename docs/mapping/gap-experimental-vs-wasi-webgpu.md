...
| 横切 | 现状 | 目标（长期） |
|------|------|----------------|
| WIT resource + `[method]` 名 | 扁平函数 + u32；**W3 高频面已挂** `gpu` / `gpu-adapter` / `gpu-device` / `gpu-command-encoder` / `gpu-render-pass-encoder` / `gpu-queue` 及对应 `[method]`；**W3+** `[method]gpu-device.create-buffer` / `[method]gpu-device.create-texture` / `[method]gpu-device.create-sampler`（返回仍 u32 / void） | 提案 resource 表；rep 仍可 u32 对齐 L2；descriptor / `list` / `option` 仍后 |
...
| Package 字符串 | experimental + W1 双注册 `wasi:webgpu/webgpu@0.3.0-rc.2`（过渡扁平 + **W3** `[method]gpu.request-adapter` / `[method]gpu-adapter.request-device` / `[method]gpu-device.queue` / `[method]gpu-device.create-command-encoder` / **W3+** `[method]gpu-device.create-buffer` / `[method]gpu-device.create-texture` / `[method]gpu-device.create-sampler`） | 收敛到更多 `[method]gpu.*` resource 名 |
