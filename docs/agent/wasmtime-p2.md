...
A third party can read §2 / §3 of the tracking table and know: pin, last-checked date, upstream latest at check, and the next allowed upgrade path (patch vs RFC). Not a `wasmtime-wasi` crate. Not a WASI 0.3 re-cut. Not Maven Central (Central waits for **P010-PUB**).
