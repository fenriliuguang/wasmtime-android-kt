...
- **D 真机/WG-5：** 用户点名才做。不新挂 WIT；Dawn 一条 render 或 compute；Android 新信息才上游开 issue 并写路线图 Upstream 一行。无 CTS 宣称

禁止：WebFetch WIT、无 offset 读 `cm.rs`/`jvm.rs`、改 hub 文件、crate `cargo fmt`、A–C 跑全量测试/仪器、wasi-gfx、新 `HostArg` 变体。复制源、白名单、窄测以[英文正文](webgpu-midterm.md)为准。
