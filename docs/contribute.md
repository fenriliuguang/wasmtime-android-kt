...
Cursor / VS Code：本仓几乎全是 Kotlin，请用 Gradle 编译，不要开 Red Hat Java（JDT LS）的 Gradle 导入与自动构建。仓库 [`.vscode/settings.json`](../.vscode/settings.json) 已关闭这些开关；否则 JDT 会把 `.kt` 当资源拷到 `runtime-jni/bin/` 等目录，删了还会再出现。
