# Agent playbook: WASI 0.3 (P1) — archived

**English** | [中文](p1-wasi-p3-playbook.zh.md)

> **Closed 2026-08-26.** Do not run this queue. Current work: [`../agent/wasmtime-p2.md`](../agent/wasmtime-p2.md). Close-out: [`p1-wasi-p3.md`](p1-wasi-p3.md). Leftover shapes: [`../mapping/gap-wasi-p3-wit.md`](../mapping/gap-wasi-p3-wit.md) (named-only).

P0 `wasi:webgpu` is **closed**. Close-out: [`p0-wasi-webgpu.md`](p0-wasi-webgpu.md). WebGPU holes: [`../mapping/gap-webgpu-wit-androidx.md`](../mapping/gap-webgpu-wit-androidx.md). Do **not** re-cut G1–G9, F1–F9, P1–P5 guest-pipeline, or WG-6.

This queue finished **ratified WASI 0.3** on Android: official WIT (not transitional `u32` / `future<u32>` smokes) and a **device instrument** per lane. Spec surface: [`p1-wasi-p3-surface.md`](p1-wasi-p3-surface.md). Pin: WASI **0.3.0**. One lane, one PR.
...
| P1-HT1 | *(landed)* fixture no longer has `gap: handle not result<response>` | `handle -> result<response, error-code>` |
...
User prompt that worked: “follow `docs/agent/wasi-p3.md`” or name the lane (`P1-FS1`, `G-fs-shape`). Gap leftovers: [`../mapping/gap-wasi-p3-wit.md`](../mapping/gap-wasi-p3-wit.md).
