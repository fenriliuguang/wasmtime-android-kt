...
|---|-------------------------------|--------------|--------|----------|--------|
| 1 | `request-adapter` | `gpu.request-adapter` | 提案 **async**；**S2** `[method]gpu.request-adapter`：`async (borrow gpu, option<gpu-request-adapter-options>) -> option<own gpu-adapter>` | 扁平名仍注册；`get-gpu` 仍为测试构造器；Guest 传 options=none | **S2 已交付** |
| 2 | `adapter-request-device` | `gpu-adapter.request-device` | 提案 **async**；**S3** `[method]gpu-adapter.request-device`：`async (borrow gpu-adapter, option<gpu-device-descriptor>) -> result<own gpu-device, request-device-error>` | 扁平名仍注册；`get-adapter` 仍为测试构造器；Guest 传 descriptor=none | **S3 已交付** |
...
| 10 | `command-encoder-finish` | `gpu-command-encoder.finish` | 提案 **sync**；**W3** 提案名路径过渡扁平 sync；**S7** `[method]gpu-command-encoder.finish`：`(borrow, option<gpu-command-buffer-descriptor>) -> own<gpu-command-buffer>`（Guest 传 none） | 扁平名仍注册；`get-encoder` 仍为测试构造器；descriptor 本刀 none | **S7** |
| 11 | `queue-submit1` | `gpu-queue.submit` | 提案 **sync**；**W3** 提案名路径过渡扁平 sync；**S5** `[method]gpu-queue.submit` sync void（`get-queue` + `get-command-buffer` + `list<borrow<gpu-command-buffer>>`） | 扁平名仍注册；L2 仍 host-fixed `submit1`（Guest list 参与编组） | **S5** |
...
| Buffer / Texture / Sampler | 创建、map（**async** map 相关） | **S4** `[method]gpu-device.create-buffer` 已交付（Guest `gpu-buffer-descriptor` → `own<gpu-buffer>`）；**S8** `[method]gpu-device.create-sampler` / `[method]gpu-texture.create-view` 已交付（Guest `option<descriptor>` = none → `own<gpu-sampler>` / `own<gpu-texture-view>`）；**W3** `[method]gpu-device.create-texture` sync 已交付（host 固定 descriptor，仍 u32）；**W3** `[method]gpu-buffer.map-async` 真 async 已交付（host 固定 MAP_READ）；**W3** `[method]gpu-buffer.unmap` sync 已交付；`get-mapped-range` 仍后 |
...
| Command encoding | begin-compute-pass / copy-buffer / pass set-pipeline / draw / set-bind-group / set-vertex-buffer | **S8** `[method]gpu-command-encoder.begin-compute-pass` 已交付（Guest `option<descriptor>` = none → `own<gpu-compute-pass-encoder>`）；**W3** `[method]gpu-compute-pass-encoder.end` / `[method]gpu-command-encoder.copy-buffer-to-buffer` / `[method]gpu-compute-pass-encoder.set-pipeline` / `[method]gpu-compute-pass-encoder.dispatch-workgroups` / `[method]gpu-render-pass-encoder.set-pipeline` / `[method]gpu-render-pass-encoder.draw` / `[method]gpu-compute-pass-encoder.set-bind-group` / `[method]gpu-render-pass-encoder.set-bind-group` / `[method]gpu-render-pass-encoder.set-vertex-buffer` sync 已交付（host 固定 4 字节 copy / stub pipeline / 1×1×1 dispatch / vertexCount 3 / 空 bind-group index 0 / VERTEX buffer slot 0，仍 u32 / void） |
...
史实（硬闸门，不随每刀改写）：W0 差距表已交付；W1 双注册见 [`../scheme/w1-dual-register.md`](../scheme/w1-dual-register.md)；W2 adapter/device 真 async 已过闸；**W3 host-fixed `[method]` 冻结**（本表 §2 为对照，不是扩面看板）。W4 分层见 [`../scheme/w4-present-strategy.md`](../scheme/w4-present-strategy.md)。**S1 已交付**（`gpu-device.queue` → `own<gpu-queue>`）。**S2 已交付**（`gpu.request-adapter` → `option<own<gpu-adapter>>`）。**S3 已交付**（`gpu-adapter.request-device` → `result<own<gpu-device>, …>`）。**S4 已交付**（`gpu-device.create-buffer` + 真实 `gpu-buffer-descriptor` → `own<gpu-buffer>`）。**S5 已交付**（`gpu-queue.submit` + `list<borrow<gpu-command-buffer>>`）。**S6 已交付**（`gpu-device.create-command-encoder` + `option<descriptor>` = none → `own<gpu-command-encoder>`）。**S7 已交付**（`gpu-command-encoder.finish` + `option<descriptor>` = none → `own<gpu-command-buffer>`）。**S8 已交付**（同形状一批：`create-sampler` / `texture.create-view` / `begin-compute-pass`，Guest descriptor=none → 各自 `own`）。**下一刀代码：S6+**（begin-render-pass 完整 record / create-texture descriptor / map-async 正式 result 等）。
