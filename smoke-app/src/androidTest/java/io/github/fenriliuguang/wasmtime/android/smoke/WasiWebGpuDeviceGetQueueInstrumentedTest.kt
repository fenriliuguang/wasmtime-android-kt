...
/**
 * W3 first slice: guest imports transitional flat
 * `wasi:webgpu/webgpu@0.3.0-rc.2#device-get-queue` (sync getter; not
 * `[method]gpu-device.queue`) after adapter → device, via
...
class WasiWebGpuDeviceGetQueueInstrumentedTest {
    @Test
    fun guestDeviceGetQueueViaProposalNameSync() {
        val bytes =
            InstrumentationRegistry.getInstrumentation()
                .context
                .assets
                .open("w1/webgpu_device_get_queue.wasm")
                .use { it.readBytes() }
