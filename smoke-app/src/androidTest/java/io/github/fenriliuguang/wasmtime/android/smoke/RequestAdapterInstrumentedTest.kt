...
/**
 * M3: guest `run` → L1 `request-adapter` → Track A [CpuWasiWebGpuHost] → non-zero u32 rep.
 */
@RunWith(AndroidJUnit4::class)
class RequestAdapterInstrumentedTest {
    @Test
    fun guestRequestAdapterViaCpuL2() {
        val bytes =
            InstrumentationRegistry.getInstrumentation()
                .context
                .assets
                .open("m3/request_adapter.wasm")
                .use { it.readBytes() }
