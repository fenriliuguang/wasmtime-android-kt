...
/**
 * Lane D cite: canonical `[method]` render slice on [DawnWasiWebGpuHost]
 * (not Cpu). Guest chains create-buffer → create-texture → create-view →
 * encoder → begin-render-pass → end → finish → [method]gpu-queue.submit.
            ...
            DawnWasiWebGpuHost.create().use { host ->
                val bytes =
                    InstrumentationRegistry.getInstrumentation()
                        .context
                        .assets
                        .open("w1/webgpu_method_dawn_render_slice.wasm")
                        .use { it.readBytes() }
                Engine.create().use { engine ->
                    Component.compile(engine, bytes).use { component ->
                        Linker.create(engine).use { linker ->
                            Store.create(engine).use { store ->
                                ExperimentalWebGpuBridge.attachDawnRenderSlice(store, host)
                                linker.instantiate(store, component).use { instance ->
                                    val harness = instance.callRunConcurrent(store)
                                    assertEquals(
                                        "guest must complete the render slice and return harness 1",
                                        1,
                                        harness,
                                    )
                                }
