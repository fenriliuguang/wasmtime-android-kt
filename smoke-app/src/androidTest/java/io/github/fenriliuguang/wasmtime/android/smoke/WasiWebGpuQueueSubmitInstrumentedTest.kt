 ...
 * `wasi:webgpu/webgpu@0.3.0-rc.2#queue-submit1` (not
 * `[method]gpu-queue.submit`) after adapter → device → queue + encoder → finish,
 * via [ExperimentalWebGpuBridge.attachQueueSubmit1] + [callRunConcurrent]
...
class WasiWebGpuQueueSubmitInstrumentedTest {
    @Test
    fun guestQueueSubmitViaProposalNameSync() {
        val bytes =
            InstrumentationRegistry.getInstrumentation()
                .context
                .assets
                .open("w1/webgpu_queue_submit.wasm")
                .use { it.readBytes() }
