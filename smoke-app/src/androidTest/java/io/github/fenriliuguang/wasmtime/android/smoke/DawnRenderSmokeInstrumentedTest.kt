...
/**
 * M4: dedicated render smoke Guest → Track B L1 → Dawn L2 → clear + present.
 *
 * Independent of Track A cube baseline.
 */
...
class DawnRenderSmokeInstrumentedTest {
    ...
    fun guestClearPresentViaDawnL2() {
        withReadySurface { ctx ->
            runOnGpuThread("m4-dawn-render-smoke", timeoutSec = 90) {
                ...
                val host = ensureHost()
                Log.i(TAG, "GpuThread: windowFromSurface ${ctx.width}x${ctx.height}")
                val windowHandle = Util.windowFromSurface(ctx.surface)
                val bytes =
                    InstrumentationRegistry.getInstrumentation()
                        .context
                        .assets
                        .open("m4/render_smoke.wasm")
                        .use { it.readBytes() }

                Log.i(TAG, "GpuThread: compile+instantiate (${bytes.size} bytes)")
                Engine.create().use { engine ->
                    Component.compile(engine, bytes).use { component ->
                        Linker.create(engine).use { linker ->
                            Store.create(engine).use { store ->
                                ExperimentalWebGpuBridge.attachRenderSmoke(store, host)
                                linker.instantiate(store, component).use { instance ->
                                    Log.i(TAG, "GpuThread: call run-clear")
                                    val code =
                                        instance.callU64U32U32(
                                            store,
                                            "run-clear",
                                            windowHandle,
                                            ctx.width,
                                    ...
                                    assertEquals("run-clear should return 0", 0, code)
                                    Log.i(TAG, "GpuThread: run-clear ok")
                                }
                            }
                        }
                    }
    ...
    companion object {
        private const val TAG = "M4DawnRenderSmoke"
        ...
        private fun ensureHost(): DawnWasiWebGpuHost {
            synchronized(hostLock) {
                return sharedHost ?: DawnWasiWebGpuHost.create().also { sharedHost = it }
            }
        }
