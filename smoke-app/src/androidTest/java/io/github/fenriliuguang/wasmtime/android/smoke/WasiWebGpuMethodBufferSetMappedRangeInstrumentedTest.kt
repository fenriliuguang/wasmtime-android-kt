 ...
 * `[method]gpu-buffer.get-mapped-range-set-with-copy` (`result<_, error>`;
 * empty data, offset/size=none; L2 still host-fixed) via
 * [ExperimentalWebGpuBridge.attachGetMappedRange] + [callRunConcurrent].
 * Not compliance.
 */
@RunWith(AndroidJUnit4::class)
class WasiWebGpuMethodBufferSetMappedRangeInstrumentedTest {
    @Test
    fun guestBufferSetMappedRangeViaMethodNameSync() {
        val bytes =
            InstrumentationRegistry.getInstrumentation()
                .context
                .assets
                .open("w1/webgpu_method_buffer_set_mapped_range.wasm")
                .use { it.readBytes() }
                    ...
                    Linker.createWithFixtureConstructors(engine).use { linker ->
                        Store.create(engine).use { store ->
                            ...
                            linker.instantiate(store, component).use { instance ->
                                ...
                                assertEquals(
                                    "guest must return harness 1 after set-mapped-range ok",
                                    1,
                                    harness,
                                )
                            }
