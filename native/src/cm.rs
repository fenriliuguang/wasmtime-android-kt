...
fn define_host(linker: &mut Linker<HostState>) -> Result<(), String> {
    ...
    // and `[method]gpu-command-encoder.copy-buffer-to-buffer` (L2 described copy)
    // and S6+ remaining encoder recording: resolve-query-set / push-debug-group /
            ...
            .func_wrap(
                ...
                |mut caller,
                 (encoder, source, destination, copy_size): (
                    ...
                    let encoder_rep = caller.data_mut().table.get(&encoder)?.rep;
                    let buffer_rep = caller.data_mut().table.get(&source.buffer)?.rep;
            ...
            .func_wrap(
                ...
                |mut caller,
                 (encoder, source, destination, copy_size): (
                    ...
                    let encoder_rep = caller.data_mut().table.get(&encoder)?.rep;
                    let texture_rep = caller.data_mut().table.get(&source.texture)?.rep;
                    let buffer_rep = caller.data_mut().table.get(&destination.buffer)?.rep;
                    let cb = caller
                        .data()
                        .experimental_host_cb
                        .as_ref()
