...
//! (resource self, sync; stub pass 79).
...
fn wasi_webgpu_method_begin_compute_pass_smoke() -> wasmtime::Result<()> {
    ...
    assert_eq!(v, 79, "guest run must return stub compute-pass rep via [method]");
    Ok(())
}
...
fn wasi_webgpu_method_begin_compute_pass_call_async() -> wasmtime::Result<()> {
    ...
    assert_eq!(
        v, 79,
        "guest run must return stub compute-pass rep via [method] call_async"
    );
    Ok(())
}
