//! W3+ slice: `get-device` + `[method]gpu-device.create-bind-group-layout`. Stub returns 59.
...
fn wasi_webgpu_method_create_bind_group_layout_smoke() -> wasmtime::Result<()> {
    ...
    assert_eq!(v, 59, "guest run must return stub bind-group-layout rep via [method]");
    Ok(())
}
...
fn wasi_webgpu_method_create_bind_group_layout_call_async() -> wasmtime::Result<()> {
    ...
    assert_eq!(v, 59, "guest run must return stub bind-group-layout rep via [method] call_async");
    Ok(())
}
