//! WASI 0.3: wasi:filesystem `append-via-stream` (two appends must not truncate).
    ...
    thread_local! {
        static DIR: PathBuf = {
            std::env::temp_dir()
                ...
                .join(format!("append-{:?}", std::thread::current().id()))
        };
    }
    DIR.with(|p| p.clone())
}
...
#[test]
fn wasi_filesystem_append_via_stream_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/wasi/filesystem_append.wasm"
    ))?;
