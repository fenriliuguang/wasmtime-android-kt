...
fn register_method_buffer_unmap(
...
#[test]
fn wasi_webgpu_method_buffer_unmap_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_buffer_unmap.wasm"
    ))?;
    let component = Component::new(&engine, bytes)?;
    let unmapped = Arc::new(AtomicBool::new(false));

    let mut linker: Linker<TestHost> = Linker::new(&engine);
    register_method_buffer_unmap(&mut linker, unmapped.clone())?;
...
#[test]
fn wasi_webgpu_method_buffer_unmap_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
