//! W3 slice: wasi:webgpu/webgpu@0.3.0-rc.2#command-encoder-begin-render-pass-clear
//! (transitional flat, sync). Stub host: request-adapter → 7, adapter-request-device → 11,
...
fn register_adapter_device_begin_render_pass(linker: &mut Linker<()>) -> wasmtime::Result<()> {
...
#[test]
fn wasi_webgpu_begin_render_pass_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_begin_render_pass.wasm"
    ))?;
    let component = Component::new(&engine, bytes)?;

    let mut linker: Linker<()> = Linker::new(&engine);
    register_adapter_device_begin_render_pass(&mut linker)?;
...
#[test]
fn wasi_webgpu_begin_render_pass_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
