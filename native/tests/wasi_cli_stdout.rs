...
#[test]
fn wasi_cli_stdout_write_via_stream_invalid_utf8_is_io() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
    let bytes = std::fs::read(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../fixtures/wasi/cli_stdout_io.wasm"
    ))?;
    let component = Component::new(&engine, bytes)?;
    ...
    let func = instance.get_typed_func::<(), (u32,)>(&mut store, "run")?;
    let (n,) = pollster::block_on(func.call_async(&mut store, ()))?;
    assert_eq!(n, 1, "guest must see error-code.io and return 1");
    Ok(())
}
