//! W3 slice: wasi:webgpu/webgpu@0.3.0-rc.2#command-encoder-finish (transitional flat, sync).
//! Stub host: request-adapter → 7, adapter-request-device → 11,
//! device-create-command-encoder → 17, command-encoder-finish → 19.

use futures::channel::oneshot;
use wasmtime::component::{Component, Linker};
use wasmtime::{Config, Engine, Store};

fn register_adapter_device_encoder_finish(linker: &mut Linker<()>) -> wasmtime::Result<()> {
...
#[test]
fn wasi_webgpu_command_encoder_finish_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_command_encoder_finish.wasm"
    ))?;
    let component = Component::new(&engine, bytes)?;

    let mut linker: Linker<()> = Linker::new(&engine);
    register_adapter_device_encoder_finish(&mut linker)?;
...
#[test]
fn wasi_webgpu_command_encoder_finish_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
