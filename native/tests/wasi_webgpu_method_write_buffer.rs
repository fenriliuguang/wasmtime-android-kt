//! W3+: `get-queue` + `[method]gpu-queue.write-buffer` (self, buffer u32). Guest returns 31.
...
fn register_method_write_buffer(
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (queue, buffer): (Resource<GpuQueue>, u32)| {
...
fn wasi_webgpu_method_write_buffer_smoke() -> wasmtime::Result<()> {
    ...
    assert_eq!(v, 31, "guest run must return stub buffer 31");
    assert!(written.load(Ordering::SeqCst), "write-buffer must have been called");
    Ok(())
}
