    ...
    webgpu.func_wrap(
        ...
        move |mut caller,
            ...
            caller
                ...
                .map(|_| ())?;
            assert!(source.mip_level.is_none() && destination.mip_level.is_none());
            assert!(source.origin.is_none() && destination.origin.is_none());
            assert!(source.aspect.is_none() && destination.aspect.is_none());
...
#[test]
fn wasi_webgpu_method_copy_texture_to_texture_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_copy_texture_to_texture.wasm"
    ))?;
    ...
    assert_eq!(
        v, 1,
        "guest run must return harness 1 after copy-texture-to-texture"
    );
    assert!(
        called.load(Ordering::SeqCst),
        "copy-texture-to-texture must have been called"
    );
    Ok(())
}

#[test]
fn wasi_webgpu_method_copy_texture_to_texture_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
