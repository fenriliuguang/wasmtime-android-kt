//! S6+: `[constructor]record-option-gpu-size64` + `[method]record-option-gpu-size64.entries`
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (record,): (Resource<RecordOptionGpuSize64>,)| {
...
#[test]
fn wasi_webgpu_method_record_option_gpu_size64_entries_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_record_option_gpu_size64_entries.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_record_option_gpu_size64_entries_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
