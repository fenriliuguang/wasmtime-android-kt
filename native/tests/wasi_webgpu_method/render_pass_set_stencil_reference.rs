...
//! WIT: `(borrow, reference: u32)`. Guest passes 0; harness 1.
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (pass, reference): (Resource<GpuRenderPassEncoder>, u32)| {
...
#[test]
fn wasi_webgpu_method_render_pass_set_stencil_reference_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_render_pass_set_stencil_reference.wasm"
    ))?;
    ...
    assert_eq!(
        v, 1,
        "guest run must return harness 1 after set-stencil-reference"
    );
    assert!(
        called.load(Ordering::SeqCst),
        "set-stencil-reference must have been called"
    );
    Ok(())
}

#[test]
fn wasi_webgpu_method_render_pass_set_stencil_reference_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
