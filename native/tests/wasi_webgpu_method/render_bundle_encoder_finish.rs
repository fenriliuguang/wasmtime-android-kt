...
//! WIT: `(borrow, option<gpu-render-bundle-descriptor>) -> own<gpu-render-bundle>`.
//! Guest passes none; drops own; harness 1.
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller,
            ...
            let resource = caller.data_mut().table.push(GpuRenderBundle)?;
            Ok((resource,))
        },
    )?;
    Ok(())
...
#[test]
fn wasi_webgpu_method_render_bundle_encoder_finish_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_render_bundle_encoder_finish.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_render_bundle_encoder_finish_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
