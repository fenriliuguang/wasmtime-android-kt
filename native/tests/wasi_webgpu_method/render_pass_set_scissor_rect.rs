...
//! WIT: `(borrow, x/y/width/height: u32)`. Guest passes 0,0,1,1; harness 1.
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (pass, x, y, width, height): (Resource<GpuRenderPassEncoder>, u32, u32, u32, u32)| {
            caller.data_mut().table.get(&pass).map(|_| ())?;
            assert_eq!((x, y, width, height), (0, 0, 1, 1));
...
#[test]
fn wasi_webgpu_method_render_pass_set_scissor_rect_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_render_pass_set_scissor_rect.wasm"
    ))?;
    ...
    assert_eq!(
        v, 1,
        "guest run must return harness 1 after set-scissor-rect"
    );
    assert!(
        called.load(Ordering::SeqCst),
        "set-scissor-rect must have been called"
    );
    Ok(())
}

#[test]
fn wasi_webgpu_method_render_pass_set_scissor_rect_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
