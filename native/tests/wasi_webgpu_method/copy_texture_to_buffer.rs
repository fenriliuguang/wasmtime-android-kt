...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller,
            ...
            caller.data_mut().table.get(&source.texture).map(|_| ())?;
            caller
                .data_mut()
                .table
                .get(&destination.buffer)
                .map(|_| ())?;
            assert!(source.mip_level.is_none());
            assert!(source.origin.is_none());
            assert!(source.aspect.is_none());
            assert!(destination.offset.is_none());
            assert!(destination.bytes_per_row.is_none());
            assert!(destination.rows_per_image.is_none());
...
#[test]
fn wasi_webgpu_method_copy_texture_to_buffer_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_copy_texture_to_buffer.wasm"
    ))?;
    ...
    assert_eq!(
        v, 1,
        "guest run must return harness 1 after copy-texture-to-buffer"
    );
    assert!(
        called.load(Ordering::SeqCst),
        "copy-texture-to-buffer must have been called"
    );
    Ok(())
}

#[test]
fn wasi_webgpu_method_copy_texture_to_buffer_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
