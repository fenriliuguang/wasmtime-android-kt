//! L2: get-supported-limits + `[method]gpu-supported-limits.max-color-attachment-bytes-per-sample`
...
#[test]
fn wasi_webgpu_method_supported_limits_max_color_attachment_bytes_per_sample_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_supported_limits_max_color_attachment_bytes_per_sample.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_supported_limits_max_color_attachment_bytes_per_sample_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
