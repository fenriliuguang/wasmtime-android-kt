...
#[test]
fn wasi_webgpu_method_supported_limits_max_color_attachment_bytes_per_sample_smoke(
) -> wasmtime::Result<()> {
...
#[test]
fn wasi_webgpu_method_supported_limits_max_color_attachment_bytes_per_sample_call_async(
) -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
