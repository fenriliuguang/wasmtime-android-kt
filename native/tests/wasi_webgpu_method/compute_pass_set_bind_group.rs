//! L2: `get-compute-pass` + `get-bind-group` +
...
fn register_method_compute_pass_set_bind_group(
...
#[test]
fn wasi_webgpu_method_compute_pass_set_bind_group_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_compute_pass_set_bind_group.wasm"
    ))?;
    let component = Component::new(&engine, bytes)?;
    let set = Arc::new(AtomicBool::new(false));

    let mut linker: Linker<TestHost> = Linker::new(&engine);
    register_method_compute_pass_set_bind_group(&mut linker, set.clone())?;
...
#[test]
fn wasi_webgpu_method_compute_pass_set_bind_group_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
