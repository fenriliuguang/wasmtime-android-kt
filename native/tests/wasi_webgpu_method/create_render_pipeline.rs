//! W3+ slice: `get-device` + `[method]gpu-device.create-render-pipeline`. Stub returns 71.
...
#[test]
fn wasi_webgpu_method_create_render_pipeline_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_create_render_pipeline.wasm"
    ))?;
    ...
    assert_eq!(v, 71, "guest run must return stub render-pipeline rep via [method]");
    Ok(())
}

#[test]
fn wasi_webgpu_method_create_render_pipeline_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
    ...
    assert_eq!(v, 71, "guest run must return stub render-pipeline rep via [method] call_async");
    Ok(())
}
