...
fn register_method_create_render_pipeline(linker: &mut Linker<TestHost>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        |mut caller, (device, descriptor): (Resource<GpuDevice>, GpuRenderPipelineDescriptor)| {
...
fn wasi_webgpu_method_create_render_pipeline_smoke() -> wasmtime::Result<()> {
    ...
    assert_eq!(
        v, 1,
        "guest run must drop own<gpu-render-pipeline> and return harness 1"
    );
    Ok(())
}
...
fn wasi_webgpu_method_create_render_pipeline_call_async() -> wasmtime::Result<()> {
    ...
    assert_eq!(
        v, 1,
        "guest run must drop own<gpu-render-pipeline> and return harness 1 via call_async"
    );
    Ok(())
}
