//! S6+: `get-buffer` + `[method]gpu-buffer.get-mapped-range-get-with-copy`
//! WIT: `(borrow<gpu-buffer>, option<u64>, option<u64>)
//!      -> result<list<u8>, get-mapped-range-error>`.
...
fn register_method_buffer_get_mapped_range(
    ...
    webgpu.func_wrap(
        ...
        move |mut caller,
              (buffer, offset, size): (Resource<GpuBuffer>, Option<u64>, Option<u64>)| {
...
#[test]
fn wasi_webgpu_method_buffer_get_mapped_range_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_buffer_get_mapped_range.wasm"
    ))?;
    let component = Component::new(&engine, bytes)?;
    let called = Arc::new(AtomicBool::new(false));
    let mut linker: Linker<TestHost> = Linker::new(&engine);
    register_method_buffer_get_mapped_range(&mut linker, called.clone())?;
    ...
    assert_eq!(
        v, 1,
        "guest run must return harness 1 after get-mapped-range ok"
    );
    assert!(
        called.load(Ordering::SeqCst),
        "get-mapped-range-get-with-copy must have been called"
    );
    Ok(())
}

#[test]
fn wasi_webgpu_method_buffer_get_mapped_range_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
