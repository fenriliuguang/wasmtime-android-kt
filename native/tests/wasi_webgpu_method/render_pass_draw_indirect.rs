//! L2: `get-pass` + `get-buffer` + `[method]gpu-render-pass-encoder.draw-indirect`
...
#[test]
fn wasi_webgpu_method_render_pass_draw_indirect_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_render_pass_draw_indirect.wasm"
    ))?;
    ...
    assert_eq!(v, 1, "guest run must return harness 1 after draw-indirect");
    assert!(
        called.load(Ordering::SeqCst),
        "draw-indirect must have been called"
    );
    Ok(())
}

#[test]
fn wasi_webgpu_method_render_pass_draw_indirect_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
