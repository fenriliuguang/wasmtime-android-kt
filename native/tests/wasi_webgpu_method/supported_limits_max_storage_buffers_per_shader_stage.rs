...
#[test]
fn wasi_webgpu_method_supported_limits_max_storage_buffers_per_shader_stage_smoke(
) -> wasmtime::Result<()> {
...
#[test]
fn wasi_webgpu_method_supported_limits_max_storage_buffers_per_shader_stage_call_async(
) -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
