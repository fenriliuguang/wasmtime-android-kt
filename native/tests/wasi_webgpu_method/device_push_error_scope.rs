...
//! WIT: `(borrow, gpu-error-filter)`. Guest validation filter; harness 1.
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (device, filter): (Resource<GpuDevice>, GpuErrorFilter)| {
            caller.data_mut().table.get(&device).map(|_| ())?;
            assert!(matches!(filter, GpuErrorFilter::Validation));
...
#[test]
fn wasi_webgpu_method_device_push_error_scope_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_device_push_error_scope.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_device_push_error_scope_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
