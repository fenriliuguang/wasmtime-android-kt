//! L2: `get-texture-view` + `[method]gpu-texture-view.label`
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (view,): (Resource<GpuTextureView>,)| {
            caller.data_mut().table.get(&view).map(|_| ())?;
...
#[test]
fn wasi_webgpu_method_texture_view_label_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_texture_view_label.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_texture_view_label_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
