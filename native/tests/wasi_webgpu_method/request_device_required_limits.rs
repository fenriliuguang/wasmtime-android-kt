//! L2 leftover: `get-adapter` + `record-option-gpu-size64` + `[method]gpu-adapter.request-device`
//! WIT: async (borrow<gpu-adapter>, option<gpu-device-descriptor>)
//!      -> result<own<gpu-device>, request-device-error>
//! Guest passes required-limits some (key=`max-bind-groups`=4) + label=`l2`; drops own on ok; harness 1.
    ...
    webgpu.func_wrap_concurrent(
        ...
        |accessor, (adapter, descriptor): (Resource<GpuAdapter>, Option<GpuDeviceDescriptor>)| {
            Box::pin(async move {
                ...
                let (key, value) = accessor.with(|mut access| {
                    access
                        .data_mut()
                        .added
                        .get(&rec.rep())
...
#[test]
fn wasi_webgpu_method_request_device_required_limits_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_request_device_required_limits.wasm"
    ))?;
    let component = Component::new(&engine, bytes)?;

    let mut linker: Linker<TestHost> = Linker::new(&engine);
    register_method_request_device_required_limits(&mut linker)?;
...
#[test]
fn wasi_webgpu_method_request_device_required_limits_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
