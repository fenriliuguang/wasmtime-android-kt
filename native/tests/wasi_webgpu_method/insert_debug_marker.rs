...
//! WIT: `(borrow encoder, marker-label: string)`. Guest passes empty label; harness 1.
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (encoder, marker_label): (Resource<GpuCommandEncoder>, String)| {
            caller.data_mut().table.get(&encoder).map(|_| ())?;
            assert!(marker_label.is_empty());
...
#[test]
fn wasi_webgpu_method_insert_debug_marker_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_insert_debug_marker.wasm"
    ))?;
    ...
    assert_eq!(
        v, 1,
        "guest run must return harness 1 after insert-debug-marker"
    );
    assert!(
        called.load(Ordering::SeqCst),
        "insert-debug-marker must have been called"
    );
    Ok(())
}

#[test]
fn wasi_webgpu_method_insert_debug_marker_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
