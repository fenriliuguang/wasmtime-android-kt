...
//! WIT: async `() -> result<option<gpu-error>, pop-error-scope-error>`. Host ok/none; harness 1.
...
#[test]
fn wasi_webgpu_method_device_pop_error_scope_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_device_pop_error_scope.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_device_pop_error_scope_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
