...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (encoder,): (Resource<GpuCommandEncoder>,)| {
...
#[test]
fn wasi_webgpu_method_pop_debug_group_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_pop_debug_group.wasm"
    ))?;
    ...
    assert_eq!(
        v, 1,
        "guest run must return harness 1 after pop-debug-group"
    );
    assert!(
        called.load(Ordering::SeqCst),
        "pop-debug-group must have been called"
    );
    Ok(())
}

#[test]
fn wasi_webgpu_method_pop_debug_group_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
