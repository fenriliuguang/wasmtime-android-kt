//! L2: `get-compilation-message` + `[method]gpu-compilation-message.message`
//! WIT: `(borrow) -> string`. Host returns Cpu stub `cpu-compilation-message`; harness 1.
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (msg,): (Resource<GpuCompilationMessage>,)| {
            ...
            Ok(("cpu-compilation-message".to_string(),))
        },
    )?;
    Ok(())
}
...
#[test]
fn wasi_webgpu_method_compilation_message_message_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_compilation_message_message.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_compilation_message_message_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
