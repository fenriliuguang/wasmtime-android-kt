//! W3+ slice: `get-device` + `[method]gpu-device.create-bind-group`. Stub returns 67.
...
fn wasi_webgpu_method_create_bind_group_smoke() -> wasmtime::Result<()> {
    ...
    assert_eq!(v, 67, "guest run must return stub bind-group rep via [method]");
    Ok(())
}
...
fn wasi_webgpu_method_create_bind_group_call_async() -> wasmtime::Result<()> {
    ...
    assert_eq!(v, 67, "guest run must return stub bind-group rep via [method] call_async");
    Ok(())
}
