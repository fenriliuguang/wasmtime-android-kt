...
//! WIT: `(borrow, x/y/width/height/min-depth/max-depth: f32)`.
//! Guest passes 0,0,1,1,0,1; `run` returns harness 1.
...
#[test]
fn wasi_webgpu_method_render_pass_set_viewport_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_render_pass_set_viewport.wasm"
    ))?;
    ...
    assert_eq!(v, 1, "guest run must return harness 1 after set-viewport");
    assert!(
        called.load(Ordering::SeqCst),
        "set-viewport must have been called"
    );
    Ok(())
}

#[test]
fn wasi_webgpu_method_render_pass_set_viewport_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
