...
//! WIT: async `()`. Host no-op; harness 1.
...
#[test]
fn wasi_webgpu_method_queue_on_submitted_work_done_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_queue_on_submitted_work_done.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_queue_on_submitted_work_done_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
