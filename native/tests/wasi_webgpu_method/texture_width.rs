//! L2: `get-texture` + `[method]gpu-texture.width`
//! WIT: (borrow) -> u32. Host returns stub texture width (1); harness 1.
...
#[test]
fn wasi_webgpu_method_texture_width_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_texture_width.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_texture_width_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
