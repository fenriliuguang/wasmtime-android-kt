//! L2: `get-adapter-info` + `[method]gpu-adapter-info.architecture`
//! WIT: `(borrow) -> string`. Host returns Cpu stub `cpu-arch`; harness 1.
...
#[test]
fn wasi_webgpu_method_adapter_info_architecture_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_adapter_info_architecture.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_adapter_info_architecture_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
