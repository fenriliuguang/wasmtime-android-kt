//! L2: `get-pipeline-layout` + `[method]gpu-pipeline-layout.label`
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap("get-pipeline-layout", |mut store, ()| {
        let resource = store.data_mut().table.push(GpuPipelineLayout)?;
        Ok((resource,))
    })?;
    webgpu.func_wrap(
        "[method]gpu-pipeline-layout.label",
        move |mut caller, (layout,): (Resource<GpuPipelineLayout>,)| {
...
#[test]
fn wasi_webgpu_method_pipeline_layout_label_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_pipeline_layout_label.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_pipeline_layout_label_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
