//! W3+: `get-queue` + `[method]gpu-queue.write-texture` (self, texture u32). Guest returns 37.
...
fn register_method_write_texture(
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (queue, texture): (Resource<GpuQueue>, u32)| {
            caller.data_mut().table.get(&queue).map(|_| ())?;
            assert_eq!(texture, 37, "guest must pass stub texture 37");
...
fn wasi_webgpu_method_write_texture_smoke() -> wasmtime::Result<()> {
    ...
    assert_eq!(v, 37, "guest run must return stub texture 37");
    assert!(written.load(Ordering::SeqCst), "write-texture must have been called");
    Ok(())
}
...
fn wasi_webgpu_method_write_texture_call_async() -> wasmtime::Result<()> {
    ...
    assert_eq!(v, 37, "guest run must return stub texture via call_async");
    assert!(written.load(Ordering::SeqCst), "write-texture must have been called");
    Ok(())
}
