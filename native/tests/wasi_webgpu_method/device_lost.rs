...
//! WIT: `(borrow) -> future<gpu-device-lost-info>`. Await; drop own; harness 1.

use std::sync::atomic::{AtomicBool, Ordering};
use std::sync::Arc;

use wasmtime::component::{Component, FutureReader, Linker, Resource, ResourceTable, ResourceType};
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (device,): (Resource<GpuDevice>,)| {
...
#[test]
fn wasi_webgpu_method_device_lost_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_device_lost.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_device_lost_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
