//! L2: `get-command-buffer` + `[method]gpu-command-buffer.set-label`
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (buffer, label): (Resource<GpuCommandBuffer>, String)| {
...
#[test]
fn wasi_webgpu_method_command_buffer_set_label_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_command_buffer_set_label.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_command_buffer_set_label_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
