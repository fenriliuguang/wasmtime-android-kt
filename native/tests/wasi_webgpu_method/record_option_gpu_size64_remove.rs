//! L2: `[constructor]record-option-gpu-size64` + `[method]record-option-gpu-size64.remove`
//! WIT: described mutate; guest empty key; harness 1.
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (record, key): (Resource<RecordOptionGpuSize64>, String)| {
...
#[test]
fn wasi_webgpu_method_record_option_gpu_size64_remove_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_record_option_gpu_size64_remove.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_record_option_gpu_size64_remove_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
