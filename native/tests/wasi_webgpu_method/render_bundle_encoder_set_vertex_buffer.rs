...
//! Guest passes slot=0, buffer=some, offset/size=none; harness 1.
...
#[test]
fn wasi_webgpu_method_render_bundle_encoder_set_vertex_buffer_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_render_bundle_encoder_set_vertex_buffer.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_render_bundle_encoder_set_vertex_buffer_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
