...
#[test]
fn wasi_webgpu_method_device_limits_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_device_limits.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_device_limits_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
