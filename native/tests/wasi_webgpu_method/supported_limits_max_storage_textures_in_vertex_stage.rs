//! S6+: `get-supported-limits` + `[method]gpu-supported-limits.max-storage-textures-in-vertex-stage`
...
#[test]
fn wasi_webgpu_method_supported_limits_max_storage_textures_in_vertex_stage_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_supported_limits_max_storage_textures_in_vertex_stage.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_supported_limits_max_storage_textures_in_vertex_stage_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
