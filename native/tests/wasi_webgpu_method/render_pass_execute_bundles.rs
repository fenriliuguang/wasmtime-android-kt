...
//! WIT: `(borrow, list<borrow<gpu-render-bundle>>)`. One-element list; harness 1.
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap("get-render-bundle", |mut store, ()| {
        let resource = store.data_mut().table.push(GpuRenderBundle)?;
        Ok((resource,))
    })?;
    webgpu.func_wrap(
        "[method]gpu-render-pass-encoder.execute-bundles",
        move |mut caller,
            ...
            caller.data_mut().table.get(&bundles[0]).map(|_| ())?;
...
#[test]
fn wasi_webgpu_method_render_pass_execute_bundles_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_render_pass_execute_bundles.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_render_pass_execute_bundles_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
