//! Named leftover: `get-adapter` + `record-option-gpu-size64` + `[method]gpu-adapter.request-device`
//! Guest adds F9-skipped stage-only storage keys (vertex/fragment buffers+textures = 4);
//! drops own on ok; harness 1.
...
struct TestHost {
    table: ResourceTable,
    added: HashMap<u32, HashMap<String, Option<u64>>>,
}

fn register_method_request_device_stage_storage_limits(
    ...
    webgpu.func_wrap("[constructor]record-option-gpu-size64", |mut store, ()| {
    ...
    webgpu.func_wrap_concurrent(
        ...
        |accessor, (adapter, descriptor): (Resource<GpuAdapter>, Option<GpuDeviceDescriptor>)| {
            Box::pin(async move {
                ...
                let desc = descriptor
                    .as_ref()
                    .expect("guest must pass descriptor=some");
                assert!(desc.required_features.is_none());
                let rec = desc
                    .required_limits
                    .as_ref()
                    .expect("guest must pass required-limits record");
                accessor.with(|mut access| access.data_mut().table.get(rec).map(|_| ()))?;
                let added = accessor.with(|mut access| {
                    access
                        .data_mut()
                        .added
                        .get(&rec.rep())
                        .cloned()
                        .expect("guest must add before request-device")
                });
                assert_eq!(
                    added.get("max-storage-buffers-in-vertex-stage"),
                    Some(&Some(4))
                );
                assert_eq!(
                    added.get("max-storage-buffers-in-fragment-stage"),
                    Some(&Some(4))
                );
                assert_eq!(
                    added.get("max-storage-textures-in-vertex-stage"),
                    Some(&Some(4))
                );
                assert_eq!(
                    added.get("max-storage-textures-in-fragment-stage"),
                    Some(&Some(4))
                );
                ...
                let resource = accessor
                    .with(|mut access| access.data_mut().table.push(GpuDevice { rep: 12 }))?;
...
#[test]
fn wasi_webgpu_method_request_device_stage_storage_limits_smoke() -> wasmtime::Result<()> {
    ...
    let wat = std::fs::read_to_string(concat!(
        ...
        "/../fixtures/w1/webgpu_method_request_device_stage_storage_limits.wat"
    ))?;
    assert!(
        wat.contains("max-storage-buffers-in-vertex-stage")
            && wat.contains("max-storage-buffers-in-fragment-stage")
            && wat.contains("max-storage-textures-in-vertex-stage")
            && wat.contains("max-storage-textures-in-fragment-stage"),
        "fixture must add the four stage-only storage limit keys"
    );
    let bytes = std::fs::read(concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../fixtures/w1/webgpu_method_request_device_stage_storage_limits.wasm"
    ))?;
    let component = Component::new(&engine, bytes)?;

    let mut linker: Linker<TestHost> = Linker::new(&engine);
    register_method_request_device_stage_storage_limits(&mut linker)?;
...
#[test]
fn wasi_webgpu_method_request_device_stage_storage_limits_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
