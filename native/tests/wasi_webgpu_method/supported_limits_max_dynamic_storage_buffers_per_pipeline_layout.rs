//! S6+: `get-supported-limits` + `[method]gpu-supported-limits.max-dynamic-storage-buffers-per-pipeline-layout`
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (limits,): (Resource<GpuSupportedLimits>,)| {
            caller.data_mut().table.get(&limits).map(|_| ())?;
...
#[test]
fn wasi_webgpu_method_supported_limits_max_dynamic_storage_buffers_per_pipeline_layout_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_supported_limits_max_dynamic_storage_buffers_per_pipeline_layout.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_supported_limits_max_dynamic_storage_buffers_per_pipeline_layout_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
