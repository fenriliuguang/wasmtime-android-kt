//! L2: `get-compute-pipeline` + `[method]gpu-compute-pipeline.set-label`
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (pipeline, label): (Resource<GpuComputePipeline>, String)| {
...
#[test]
fn wasi_webgpu_method_compute_pipeline_set_label_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_compute_pipeline_set_label.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_compute_pipeline_set_label_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
