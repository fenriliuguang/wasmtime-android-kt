//! L2: `get-adapter-info` + `[method]gpu-adapter-info.subgroup-max-size`
//! WIT: `(borrow) -> u32`. Host returns Cpu stub 128; harness 1.
...
#[test]
fn wasi_webgpu_method_adapter_info_subgroup_max_size_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_adapter_info_subgroup_max_size.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_adapter_info_subgroup_max_size_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
