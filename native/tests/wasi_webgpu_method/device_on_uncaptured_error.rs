...
//! WIT: `(borrow) -> stream<gpu-error>`. Drop readable; harness 1.

use std::sync::atomic::{AtomicBool, Ordering};
use std::sync::Arc;

use wasmtime::component::{Component, Linker, Resource, ResourceTable, ResourceType, StreamReader};
...
#[test]
fn wasi_webgpu_method_device_on_uncaptured_error_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_device_on_uncaptured_error.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_device_on_uncaptured_error_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
