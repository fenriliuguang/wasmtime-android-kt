//! L2: `get-query-set` + `[method]gpu-query-set.count`
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (query_set,): (Resource<GpuQuerySet>,)| {
...
#[test]
fn wasi_webgpu_method_query_set_count_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_query_set_count.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_query_set_count_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
