//! L2: `get-texture` + `[method]gpu-texture.texture-binding-view-dimension`
//! WIT: `(borrow) -> option<gpu-texture-view-dimension>`. Host none; harness 1.
...
#[test]
fn wasi_webgpu_method_texture_texture_binding_view_dimension_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_texture_texture_binding_view_dimension.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_texture_texture_binding_view_dimension_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
