//! L2: `get-compute-pass` + `[method]gpu-compute-pass-encoder.set-label`
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (pass, label): (Resource<GpuComputePassEncoder>, String)| {
...
#[test]
fn wasi_webgpu_method_compute_pass_set_label_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_compute_pass_set_label.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_compute_pass_set_label_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
