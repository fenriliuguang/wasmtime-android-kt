//! L2: `get-compilation-message` + `[method]gpu-compilation-message.length`
//! WIT: `(borrow) -> u64`. Host returns Cpu stub 256; harness 1.
...
#[test]
fn wasi_webgpu_method_compilation_message_length_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_compilation_message_length.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_compilation_message_length_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
