//! L2: `get-bind-group-layout` + `[method]gpu-bind-group-layout.set-label`
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap("get-bind-group-layout", |mut store, ()| {
        let resource = store.data_mut().table.push(GpuBindGroupLayout)?;
        Ok((resource,))
    })?;
    webgpu.func_wrap(
        "[method]gpu-bind-group-layout.set-label",
        move |mut caller, (layout, label): (Resource<GpuBindGroupLayout>, String)| {
...
#[test]
fn wasi_webgpu_method_bind_group_layout_set_label_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_bind_group_layout_set_label.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_bind_group_layout_set_label_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
