...
//! WIT: `(borrow, group-label: string)`. Guest passes empty label; harness 1.
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (encoder, group_label): (Resource<GpuRenderBundleEncoder>, String)| {
...
#[test]
fn wasi_webgpu_method_render_bundle_encoder_push_debug_group_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_render_bundle_encoder_push_debug_group.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_render_bundle_encoder_push_debug_group_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
