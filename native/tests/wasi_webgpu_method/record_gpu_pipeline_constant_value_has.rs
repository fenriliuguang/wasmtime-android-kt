//! L2: `[constructor]record-gpu-pipeline-constant-value` + `[method]record-gpu-pipeline-constant-value.has`
...
#[test]
fn wasi_webgpu_method_record_gpu_pipeline_constant_value_has_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_record_gpu_pipeline_constant_value_has.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_record_gpu_pipeline_constant_value_has_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
