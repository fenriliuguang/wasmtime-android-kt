//! L2: `[constructor]record-option-gpu-size64` + `[method]record-option-gpu-size64.get`
//! WIT: described mutate; guest empty key; host none; harness 1.
...
#[test]
fn wasi_webgpu_method_record_option_gpu_size64_get_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_record_option_gpu_size64_get.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_record_option_gpu_size64_get_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
