//! L2: `get-gpu-error` + `[method]gpu-error.message`
//! WIT: `(borrow) -> string`. Host returns Cpu stub `cpu-gpu-error`; harness 1.
...
#[test]
fn wasi_webgpu_method_gpu_error_message_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_gpu_error_message.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_gpu_error_message_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
