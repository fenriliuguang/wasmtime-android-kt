...
//! WIT: `(borrow) -> own<gpu-supported-features>`. Drop own; harness 1.
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (adapter,): (Resource<GpuAdapter>,)| {
...
#[test]
fn wasi_webgpu_method_adapter_features_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_adapter_features.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_adapter_features_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
