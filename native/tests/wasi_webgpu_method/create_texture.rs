...
//! Stub: get-device pushes a host `GpuDevice`; method returns 37.
...
fn wasi_webgpu_method_create_texture_smoke() -> wasmtime::Result<()> {
    ...
    assert_eq!(v, 37, "guest run must return stub texture rep via [method]");
    Ok(())
}
...
fn wasi_webgpu_method_create_texture_call_async() -> wasmtime::Result<()> {
    ...
    assert_eq!(
        v, 37,
        "guest run must return stub texture rep via [method] call_async"
    );
    Ok(())
}
