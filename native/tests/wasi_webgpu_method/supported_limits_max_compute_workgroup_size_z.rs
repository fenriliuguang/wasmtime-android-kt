//! L2: get-supported-limits + `[method]gpu-supported-limits.max-compute-workgroup-size-z`
...
#[test]
fn wasi_webgpu_method_supported_limits_max_compute_workgroup_size_z_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_supported_limits_max_compute_workgroup_size_z.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_supported_limits_max_compute_workgroup_size_z_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
