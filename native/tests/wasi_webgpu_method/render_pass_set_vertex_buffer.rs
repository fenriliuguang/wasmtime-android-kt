...
fn register_method_render_pass_set_vertex_buffer(
...
#[test]
fn wasi_webgpu_method_render_pass_set_vertex_buffer_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_render_pass_set_vertex_buffer.wasm"
    ))?;
    let component = Component::new(&engine, bytes)?;
    let set = Arc::new(AtomicBool::new(false));

    let mut linker: Linker<TestHost> = Linker::new(&engine);
    register_method_render_pass_set_vertex_buffer(&mut linker, set.clone())?;
...
#[test]
fn wasi_webgpu_method_render_pass_set_vertex_buffer_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
