//! S6+: `get-supported-limits` + `[method]gpu-supported-limits.min-uniform-buffer-offset-alignment`
...
#[test]
fn wasi_webgpu_method_supported_limits_min_uniform_buffer_offset_alignment_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_supported_limits_min_uniform_buffer_offset_alignment.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_supported_limits_min_uniform_buffer_offset_alignment_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
