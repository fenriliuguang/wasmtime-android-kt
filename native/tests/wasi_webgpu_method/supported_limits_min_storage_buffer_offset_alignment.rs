...
#[test]
fn wasi_webgpu_method_supported_limits_min_storage_buffer_offset_alignment_smoke(
) -> wasmtime::Result<()> {
...
#[test]
fn wasi_webgpu_method_supported_limits_min_storage_buffer_offset_alignment_call_async(
) -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
