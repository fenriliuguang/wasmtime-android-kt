...
//! WIT: `(borrow, range-offset: u32, data: list<u8>, option offset/size)`.
//! Guest passes range 0, empty data, offset/size none; harness 1.
...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller,
            ...
            assert!(data_offset.is_none());
            assert!(data_size.is_none());
...
#[test]
fn wasi_webgpu_method_render_bundle_encoder_set_immediates_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_render_bundle_encoder_set_immediates.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_render_bundle_encoder_set_immediates_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
