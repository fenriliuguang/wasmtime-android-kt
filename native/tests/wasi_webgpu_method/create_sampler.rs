//! W3+ slice: `get-device` + `[method]gpu-device.create-sampler`. Stub returns 53.
...
fn wasi_webgpu_method_create_sampler_smoke() -> wasmtime::Result<()> {
    ...
    assert_eq!(v, 53, "guest run must return stub sampler rep via [method]");
    Ok(())
}
...
fn wasi_webgpu_method_create_sampler_call_async() -> wasmtime::Result<()> {
    ...
    assert_eq!(v, 53, "guest run must return stub sampler rep via [method] call_async");
    Ok(())
}
