...
fn register_method_render_pass_end(
...
#[test]
fn wasi_webgpu_method_render_pass_end_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_render_pass_end.wasm"
    ))?;
    let component = Component::new(&engine, bytes)?;
    let ended = Arc::new(AtomicBool::new(false));

    let mut linker: Linker<TestHost> = Linker::new(&engine);
    register_method_render_pass_end(&mut linker, ended.clone())?;
...
#[test]
fn wasi_webgpu_method_render_pass_end_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
