//! WG-6: `get-device` + shader + VERTEX buffer + render pipeline (float32x3)
//! + texture/view + begin-render-pass + set-pipeline + set-vertex-buffer +
//! `draw`(3) + end + finish + `queue.submit`.
//! Not 1×1 color-clear cite (`dawn_render_slice`). Not `@builtin(vertex_index)`.
...
fn register_dawn_guest_render(linker: &mut Linker<TestHost>, flags: Flags) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        |mut caller, (device, descriptor): (Resource<GpuDevice>, GpuTextureDescriptor)| {
            ...
            assert_eq!(descriptor.size.height, Some(1));
...
#[test]
fn wasi_webgpu_method_dawn_guest_render_smoke() -> wasmtime::Result<()> {
    let wat = include_str!("../../../fixtures/w1/webgpu_method_dawn_guest_render.wat");
    assert!(
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_dawn_guest_render.wasm"
    ))?;
    let component = Component::new(&engine, bytes)?;
    let flags = flags();
    let mut linker: Linker<TestHost> = Linker::new(&engine);
    register_dawn_guest_render(&mut linker, clone_flags(&flags))?;
...
#[test]
fn wasi_webgpu_method_dawn_guest_render_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
