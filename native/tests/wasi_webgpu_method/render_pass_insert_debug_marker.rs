...
fn register(linker: &mut Linker<TestHost>, called: Arc<AtomicBool>) -> wasmtime::Result<()> {
    ...
    webgpu.func_wrap(
        ...
        move |mut caller, (pass, marker_label): (Resource<GpuRenderPassEncoder>, String)| {
...
#[test]
fn wasi_webgpu_method_render_pass_insert_debug_marker_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_render_pass_insert_debug_marker.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_render_pass_insert_debug_marker_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
