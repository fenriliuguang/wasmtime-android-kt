//! L2: `get-adapter-info` + `[method]gpu-adapter-info.is-fallback-adapter`
//! WIT: `(borrow) -> bool`. Host returns Cpu stub false; harness 1.
...
#[test]
fn wasi_webgpu_method_adapter_info_is_fallback_adapter_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/w1/webgpu_method_adapter_info_is_fallback_adapter.wasm"
    ))?;
...
#[test]
fn wasi_webgpu_method_adapter_info_is_fallback_adapter_call_async() -> wasmtime::Result<()> {
    let mut config = Config::new();
    config.wasm_component_model(true);
    config.wasm_component_model_async(true);
    let engine = Engine::new(&config)?;
