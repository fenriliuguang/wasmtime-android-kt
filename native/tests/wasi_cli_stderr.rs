//! WASI 0.3: wasi:cli/stderr@0.3.0#write-via-stream smoke (official result ok path).
...
#[test]
fn wasi_cli_stderr_write_via_stream_smoke() -> wasmtime::Result<()> {
    ...
    let bytes = std::fs::read(concat!(
        ...
        "/../fixtures/wasi/cli_stderr.wasm"
    ))?;
    let component = Component::new(&engine, bytes)?;

    let mut linker = Linker::new(&engine);
    linker.instance("wasi:cli/stderr@0.3.0")?.func_wrap(
        "write-via-stream",
