...
class CpuWasiWebGpuHost : WasiWebGpuHost {
    ...
    override fun deviceLabel(handle: GpuHandle): String {
        handles.get<Device>(handle, ResourceKind.Device)
        return ""
    }
    ...
    override fun renderPassEncoderLabel(handle: GpuHandle): String {
        handles.get<RenderPassEncoder>(handle, ResourceKind.RenderPassEncoder)
        return ""
    }
    ...
    override fun renderPipelineLabel(handle: GpuHandle): String {
        handles.get<RenderPipeline>(handle, ResourceKind.RenderPipeline)
        return ""
    }
    ...
    override fun canvasContextHasConfiguration(context: Int): Int {
        ...
        val state = handles.get<CanvasContext>(GpuHandle(context), ResourceKind.CanvasContext)
        return if (state.configured) 1 else 0
    }
    ...
    private fun configuredCanvas(context: Int): CanvasContext {
        if (context == 0) {
            throw HostException.Validation("canvas context not configured")
        }
        val state = handles.get<CanvasContext>(GpuHandle(context), ResourceKind.CanvasContext)
