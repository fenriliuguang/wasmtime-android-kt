...
| Job | 内容 |
|-----|------|
| `native (cargo test)` | `native/` 下 `cargo test --locked --test m2_async_get --test p3_stream_read --test p3_stream_write --test wasi_random_u64 --test wasi_monotonic_now --test wasi_cli_stdout --test wasi_monotonic_wait_for --test wasi_webgpu_request_adapter`（CI 限并行以防 OOM） |
