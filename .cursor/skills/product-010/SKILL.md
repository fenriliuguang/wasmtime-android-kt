...
description: >-
  ...
  vsync on-frame), claim table, then publish CI). Remaining after PUB:
  P010-GFXB then P010-GFXV.
...
2. Order is the table in the playbook (SPI → DISC → FIX → CLIERR → TCP → HTTP → gfx pin/host/loop → **GFXB → GFXV** → claim → publish). Needles: [`docs/scheme/product-010.md`](docs/scheme/product-010.md). After PUB, remaining **Next:** is **P010-GFXB** until the complete loop lands.
...
4. Do not add `wasmtime-wasi` without a size + Android thread note. Do not JS-style frame callbacks. Publishing CI is **P010-PUB** (already landed); do not press when secrets are missing. Complete frame loop is **P010-GFXB** then **P010-GFXV**.
