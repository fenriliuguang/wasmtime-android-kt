...
description: >-
  ...
  already landed. Living leftover is cube hitch restart (gfx-hitch). Use when
  the user names native-dawn, ND-DISP, ND-REST, or native-dawn-remaining.
  For 下一刀 / 抖动 / hitch / 真机 follow docs/agent/gfx-hitch.md.
---
...
For **下一刀** / 抖动 / hitch / cube-pop / 真机: [`docs/agent/gfx-hitch.md`](docs/agent/gfx-hitch.md) and `python3 ./scripts/gfx-hitch-remaining.py` on **`fix/300-gfx-cube-pop`**. Forget inherited Closed/Likely; start from mapping §6 hot-path stages.

1. `python3 ./scripts/native-dawn-remaining.py` prints empty and points at gfx-hitch. Do **not** invent new `ND-*` needles. Do **not** open a consume PR.
