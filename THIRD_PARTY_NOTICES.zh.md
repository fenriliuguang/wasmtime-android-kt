...
| Dawn C API（ND-DEFAULT 产品 `.so`） | Google Android prebuilt SHA `bddf1a04…`（发包 `--prebuilt`）。配方 [`scripts/build-dawn-c-android.py`](scripts/build-dawn-c-android.py) 产出 `libwebgpu_dawn.so`（`webgpu.h`）。BSD-3-Clause：[`native/third_party/dawn-c/LICENSE`](native/third_party/dawn-c/LICENSE)。**不要提交 `.so`。** **0.1.2-SNAPSHOT** 起发包把它打进 `host-dawn` AAR。默认 APK 排除 androidx `libwebgpu_c_bundled.so`。 |
...
## 4. Vendor 的 WIT（`third_party/wasi-webgpu/`、`third_party/wasi-gfx/`）
...
| `wasi-gfx` 标签 `v0.2.0` | 该 tag **无 LICENSE 文件**。来源：[`ORIGIN.txt`](third_party/wasi-gfx/v0.2.0/ORIGIN.txt)。钉版见 [`docs/scheme/rfc.md`](docs/scheme/rfc.md)。 |
