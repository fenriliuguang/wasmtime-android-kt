...
| Dawn C API（ND-DEFAULT 产品 `.so`） | 与该 AAR 同一 Dawn SHA。配方 [`scripts/build-dawn-c-android.py`](scripts/build-dawn-c-android.py) 产出 `libwebgpu_dawn.so`（`webgpu.h`）。BSD-3-Clause：[`native/third_party/dawn-c/LICENSE`](native/third_party/dawn-c/LICENSE)。**不要提交 `.so`。** **0.1.1** 起发包把它打进 `host-dawn` AAR。默认 APK 排除 androidx `libwebgpu_c_bundled.so`。 |
